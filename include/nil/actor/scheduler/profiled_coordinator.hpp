...
#include <nil/actor/policy/profiled.hpp>
    ...
    public:
        ...
        public:
            ...
            static measurement take() {
                ...
                auto result = ::thread_info(tself, THREAD_BASIC_INFO, reinterpret_cast<thread_info_t>(&info), &count);
            ...
            friend std::ostream &operator<<(std::ostream &out, const measurement &m) {
                using std::setw;
                out << setw(15) << m.runtime.count() << setw(15) << m.usr.count() << setw(15) << m.sys.count() << m.mem;
                return out;
            }
        ...
        void init(spawner_config &cfg) override {
            ...
            auto res = get_or(cfg, "scheduler.profiling-resolution", sr::profiling_resolution);
        ...
        void record(Time t, Label label, size_t rec_id, const measurement &m) {
            using std::setw;
            file_ << setw(21) << t.time_since_epoch().count() << setw(10) << label << setw(10) << rec_id << m << '\n';
        }
