...
namespace nil {
    namespace actor {
        ...
        make_source_result_t<typename Driver::downstream_manager_type, Ts...>
            attach_stream_source(scheduled_actor *self, std::tuple<Ts...> xs, CtorArgs &&... ctor_args) {
            using namespace detail;
            auto mgr = make_stream_source<Driver>(self, std::forward<CtorArgs>(ctor_args)...);
        ...
        /// @returns The allocated `stream_manager` and the output slot.
        template<class... Ts, class Init, class Pull, class Done, class Finalize = unit_t,
        ...
        make_source_result_t<DownstreamManager, Ts...>
            attach_stream_source(scheduled_actor *self, std::tuple<Ts...> xs, Init init, Pull pull, Done done,
            ...
            return attach_stream_source<driver>(self, std::move(xs), std::move(init), std::move(pull), std::move(done),
                                                std::move(fin));
        }
        ...
        detail::enable_if_t<!is_actor_handle<Init>::value && Trait::valid, make_source_result_t<DownstreamManager>>
            attach_stream_source(scheduled_actor *self, Init init, Pull pull, Done done, Finalize fin = {},
            ...
            static_assert(detail::sendable<stream<output_type>>,
                          ...
                          "did you forgot to announce it via ACTOR_ADD_TYPE_ID?");
            return attach_stream_source(self, std::make_tuple(), init, pull, done, fin, token);
        }
        ...
        detail::enable_if_t<is_actor_handle<ActorHandle>::value, make_source_result_t<DownstreamManager>>
            attach_stream_source(scheduled_actor *self, const ActorHandle &dest, std::tuple<Ts...> xs, Init init,
                                 Pull pull, Done done, Finalize fin = {}, policy::arg<DownstreamManager> = {}) {
            using namespace detail;
            using token = type_list<stream<typename DownstreamManager::output_type>, strip_and_convert_t<Ts>...>;
            static_assert(response_type_unbox<signatures_of_t<ActorHandle>, token>::valid,
                          "receiver does not accept the stream handshake");
            using driver = detail::stream_source_driver_impl<DownstreamManager, Pull, Done, Finalize>;
            auto mgr = detail::make_stream_source<driver>(self, std::move(init), std::move(pull), std::move(done),
                                                          std::move(fin));
        ...
        detail::enable_if_t<is_actor_handle<ActorHandle>::value && Trait::valid,
            ...
            attach_stream_source(scheduled_actor *self, const ActorHandle &dest, Init init, Pull pull, Done done,
                                 Finalize fin = {}, policy::arg<DownstreamManager> token = {}) {
            return attach_stream_source(self, dest, std::make_tuple(), std::move(init), std::move(pull),
                                        std::move(done), std::move(fin), token);
        }
