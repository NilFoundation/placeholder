        ...
        namespace mixin {
            ...
            public:
                ...
                void delayed_anon_send(const Dest &dest, std::chrono::duration<Rep, Period> rel_timeout, Ts &&... xs) {
                    ...
                    auto &clock = self->system().clock();
                    auto timeout = clock.now() + rel_timeout;
                    detail::profiled_send(self, nullptr, dest, clock, timeout, make_message_id(P),
                                          std::forward<Ts>(xs)...);
                }
            ...
            private:
                ...
                template<class T = Subtype,
                         class = detail::enable_if_t<std::is_base_of<typed_actor_view_base, T>::value>>
