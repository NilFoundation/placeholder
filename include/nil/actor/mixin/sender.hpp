...
namespace nil::actor::mixin {
    ...
    public:
        ...
        void send(const group &dest, Ts &&... xs) {
            ...
            if (dest) {
                auto self = dptr();
                dest->eq_impl(make_message_id(P), self->ctrl(), self->context(), std::forward<Ts>(xs)...);
            }
        }
        ...
        detail::enable_if_t<!std::is_same<group, Dest>::value> send(const Dest &dest, Ts &&... xs) {
            ...
            auto self = dptr();
            detail::profiled_send(self, self->ctrl(), dest, make_message_id(P), {}, self->context(),
                                  std::forward<Ts>(xs)...);
        }
        ...
        void anon_send(const Dest &dest, Ts &&... xs) {
            ...
            auto self = dptr();
            detail::profiled_send(self, nullptr, dest, make_message_id(P), {}, self->context(),
                                  std::forward<Ts>(xs)...);
        }
        ...
        detail::enable_if_t<!std::is_same<Dest, group>::value>
            ...
            type_check(dest, args_token);
            auto self = dptr();
            detail::profiled_send(self, self->ctrl(), dest, self->system().clock(), timeout, make_message_id(P),
                                  std::forward<Ts>(xs)...);
        }
        ...
        void scheduled_send(const group &dest, actor_clock::time_point timeout, Ts &&... xs) {
            ...
            static_assert((detail::sendable<Ts> && ...),
                          "at least one type has no ID, "
                          "did you forgot to announce it via ACTOR_ADD_TYPE_ID?");
            static_assert(!statically_typed<Subtype>(), "statically typed actors are not allowed to send to groups");
            // TODO: consider whether it's feasible to track messages to groups
            if (dest) {
                auto self = dptr();
                auto &clock = self->system().clock();
                clock.schedule_message(timeout, dest, self->ctrl(), make_message(std::forward<Ts>(xs)...));
            }
        }
        ...
        detail::enable_if_t<!std::is_same<Dest, group>::value>
            ...
            detail::profiled_send(self, self->ctrl(), dest, clock, timeout, make_message_id(P),
                                  std::forward<Ts>(xs)...);
        }
        ...
        void scheduled_anon_send(const Dest &dest, actor_clock::time_point timeout, Ts &&... xs) {
            ...
            type_check(dest, args_token);
            auto self = dptr();
            detail::profiled_send(self, nullptr, dest, self->system().clock(), timeout, make_message_id(P),
                                  std::forward<Ts>(xs)...);
        }
        ...
        void delayed_anon_send(const Dest &dest, std::chrono::duration<Rep, Period> rel_timeout, Ts &&... xs) {
            ...
            auto &clock = self->system().clock();
            auto timeout = clock.now() + rel_timeout;
            detail::profiled_send(self, nullptr, dest, clock, timeout, make_message_id(P), std::forward<Ts>(xs)...);
        }
    ...
    private:
        ...
        detail::enable_if_t<std::is_base_of<abstract_actor, T>::value, Subtype *> dptr() {
            return static_cast<Subtype *>(this);
        }

        template<class T = Subtype, class = detail::enable_if_t<std::is_base_of<typed_actor_view_base, T>::value>>
