        ...
        namespace mixin {
            ...
            public:
                ...
                auto request(const Handle &dest, std::chrono::duration<Rep, Period> timeout, Ts &&... xs) {
                    ...
                    if (dest) {
                        detail::profiled_send(
                            self, self->ctrl(), dest, req_id, {}, self->context(), std::forward<Ts>(xs)...);
                        self->request_response_timeout(timeout, req_id);
                    } else {
                        self->eq_impl(
                            req_id.response_id(), self->ctrl(), self->context(), make_error(sec::invalid_argument));
                    }
                    using response_type = response_type_t<typename Handle::signatures,
                ...
                auto fan_out_request(const Container &destinations,
                    ...
                    if (ids.empty()) {
                        auto req_id = dptr->new_request_id(Prio);
                        dptr->eq_impl(
                            req_id.response_id(), dptr->ctrl(), dptr->context(), make_error(sec::invalid_argument));
                        ids.emplace_back(req_id.response_id());
                    }
