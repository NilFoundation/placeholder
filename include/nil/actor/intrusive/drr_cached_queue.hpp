...
#include <nil/actor/intrusive/new_round_result.hpp>
...
namespace nil {
    namespace actor {
        namespace intrusive {
            ...
            /// A Deficit Round Robin queue with an internal cache for allowing
            /// skipping consumers.
            ...
            public:
                ...
                drr_cached_queue(policy_type p) : list_(p), deficit_(0), cache_(std::move(p)) {
                    // nop
                }

                drr_cached_queue(drr_cached_queue &&other) :
                    list_(std::move(other.list_)), deficit_(other.deficit_), cache_(std::move(other.cache_)) {
                    // nop
                }
                ...
                /// Returns the accumulated size of all stored tasks in the
                /// list, i.e., tasks that are not in the cache.
                ...
                /// Applies `f` to each element in the queue, excluding cached
                /// elements.
                template<class F>
                ...
                /// Takes the first element out of the queue if the deficit
                /// allows it and returns the element.
                /// @private
                unique_pointer next() noexcept {
                    return list_.next(deficit_);
                }

                /// Takes the first element out of the queue (after flushing the
                /// cache)  and returns it, ignoring the deficit count.
                ...
                /// Consumes items from the queue until the queue is empty,
                /// there is not enough deficit to dequeue the next task or the
                /// consumer returns `stop`.
                ...
                /// Run a new round with `quantum`, dispatching all tasks to
                /// `consumer`.
                template<class F>
                new_round_result new_round(deficit_type quantum,
                                           F &consumer) noexcept(noexcept(consumer(std::declval<value_type &>()))) {
                    if (list_.empty())
                        return {false, false};
                    deficit_ += quantum;
                    long consumed = 0;
                    ...
                    do {
                        ...
                        switch (consumer_res) {
                            case task_result::skip:
                                // Fix deficit counter since we didn't actually
                                // use it.
