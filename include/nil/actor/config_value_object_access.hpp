    ...
    namespace actor {
        ...
        struct config_value_object_access {
            ...
            static bool extract(const config_value *src, object_type *dst) {
                auto dict = nil::actor::get_if<config_value::dictionary>(src);
                if (!dict)
                    return false;
                for (auto field : Trait::fields()) {
                    if (auto value = nil::actor::get_if(dict, field->name())) {
                        if (dst) {
            ...
            static object_type get(const config_value &x) {
                ...
                if (!result)
                    ACTOR_RAISE_ERROR("config_value does not contain requested object");
            ...
            static void parse_cli(string_parser_state &ps, object_type &x) {
                ...
                do {
                    ...
                    auto predicate = [&](config_value_field<object_type> *x) { return x->name() == field_name; };
