    ...
    namespace actor {
        ...
        public:
            ...
            struct mailbox_policy {
                ...
                using queue_type =
                    intrusive::wdrr_fixed_multiplexed_queue<policy::categorized, normal_queue, urgent_queue>;
            ...
            struct BOOST_SYMBOL_VISIBLE receive_while_helper {
                ...
                template<class... Ts>
                void operator()(Ts &&... xs) {
                    static_assert(sizeof...(Ts) > 0, "operator() requires at least one argument");
            ...
            do_receive_helper do_receive(Ts &&... xs) {
                ...
                auto cb = [=](receive_cond &rc) mutable { varargs_tup_receive(rc, make_message_id(), tup); };
                return {cb};
            }
            ...
            template<class... Ts>
            void varargs_tup_receive(receive_cond &rcc, message_id mid, std::tuple<Ts...> &tup) {
                ...
                using filtered =
                    typename tl_filter_not<type_list<typename std::decay<Ts>::type...>, is_timeout_or_catch_all>::type;
                filtered tk;
                behavior bhvr {apply_moved_args(make_behavior_impl, get_indices(tk), tup)};
                using tail_indices = typename il_range<tl_size<filtered>::value, sizeof...(Ts)>::type;
            ...
            /// Receives messages until either a pre- or postcheck of `rcc` fails.
            void varargs_tup_receive(receive_cond &rcc, message_id mid, std::tuple<behavior &> &tup);
            ...
            /// Receives messages until either a pre- or postcheck of `rcc` fails.
            void receive_impl(receive_cond &rcc, message_id mid, detail::blocking_behavior &bhvr);
