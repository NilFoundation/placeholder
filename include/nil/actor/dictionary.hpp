    ...
    namespace actor {
        ...
        public:
            ...
            iterator_bool_pair emplace(K &&key, T &&value) {
                ...
                if (i->first == key)
                    return {i, false};
                return {xs_.emplace_hint(i, copy(std::forward<K>(key)), V {std::forward<T>(value)}), true};
            }
            ...
            iterator emplace_hint(iterator hint, K &&key, T &&value) {
                if (hint == end() || hint->first > key)
                    return xs_.emplace(copy(std::forward<K>(key)), V {std::forward<T>(value)}).first;
                if (hint->first == key)
                    return hint;
                return xs_.emplace_hint(hint, copy(std::forward<K>(key)), V {std::forward<T>(value)});
            }
