...
#include <nil/actor/detail/pseudo_tuple.hpp>
...
namespace nil {
    namespace actor {
        ...
        public:
            ...
            virtual result invoke(detail::invoke_result_visitor &rv, type_erased_tuple &xs) = 0;
        ...
        public:
            ...
            using result_type = typename std::conditional<std::is_reference<plain_result_type>::value,
                                                          plain_result_type,
                                                          typename std::remove_const<plain_result_type>::type>::type;

            using arg_types = typename fun_trait::arg_types;

            static constexpr bool is_manipulator = detail::tl_exists<arg_types, detail::is_mutable_ref>::value;

            using pattern = typename detail::tl_map<arg_types, param_decay>::type;

            using decayed_arg_types = typename detail::tl_map<arg_types, std::decay>::type;

            using intermediate_pseudo_tuple = typename detail::tl_apply<decayed_arg_types, detail::pseudo_tuple>::type;
            ...
            trivial_match_case(F f) : match_case(make_type_token_from_list<pattern>()), fun_(std::move(f)) {
                // nop
            }
        ...
        template<class F>
        typename std::enable_if<!std::is_base_of<match_case, F>::value, std::tuple<trivial_match_case<F>>>::type
            to_match_case_tuple(F fun) {
            return std::make_tuple(std::move(fun));
        }

        template<class MatchCase>
        typename std::enable_if<std::is_base_of<match_case, MatchCase>::value, std::tuple<const MatchCase &>>::type
        ...
        const std::tuple<Ts...> &to_match_case_tuple(const std::tuple<Ts...> &x) {
            static_assert(detail::conjunction<std::is_base_of<match_case, Ts>::value...>::value,
