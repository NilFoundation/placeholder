...
namespace nil::actor {
    ...
    public:
        ...
        inline typed_response_promise(strong_actor_ptr self, mailbox_element &src) : promise_(std::move(self), src) {
            // nop
        }
        ...
        typename std::enable_if<(sizeof...(Us) > 0) || !std::is_convertible<U, error>::value,
                                typed_response_promise>::type
            deliver(U &&x, Us &&... xs) {
            static_assert(
                std::is_same<detail::type_list<Ts...>,
                             detail::type_list<typename std::decay<U>::type, typename std::decay<Us>::type...>>::value,
