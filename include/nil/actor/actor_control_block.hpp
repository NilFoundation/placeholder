...
#include <nil/actor/weak_intrusive_ptr.hpp>
...
namespace nil::actor {
    ...
    ///
    /// When allocating a new actor, =nil; Actor will always embed the user-defined
    ...
    public:
        ...
        actor_control_block(actor_id x, node_id &y, spawner *sys, data_destructor ddtor, block_destructor bdtor) :
            strong_refs(1), weak_refs(1), aid(x), nid(std::move(y)), home_system(sys), data_dtor(ddtor),
            block_dtor(bdtor) {
            // nop
        }
        ...
        static_assert(sizeof(node_id) == sizeof(void *), "sizeof(node_id) != sizeof(size_t)");
        ...
        inline abstract_actor *get() {
            ...
            return reinterpret_cast<abstract_actor *>(reinterpret_cast<intptr_t>(this) + ACTOR_CACHE_LINE_SIZE);
        }
        ...
        static actor_control_block *from(const abstract_actor *ptr) {
            // this pointer arithmetic is compile-time checked in actor_storage's ctor
            return reinterpret_cast<actor_control_block *>(reinterpret_cast<intptr_t>(ptr) - ACTOR_CACHE_LINE_SIZE);
        }
        ...
        void enqueue(strong_actor_ptr sender, message_id mid, message content, execution_unit *host);
    ...
    typename Inspector::result_type inspect(Inspector &f, strong_actor_ptr &x) {
        ...
        return f(meta::type_name("actor"), aid, meta::omittable_if_none(), nid, meta::load_callback(load),
                 meta::save_callback(save));
    }
...
namespace std {
    ...
    template<>
    struct hash<nil::actor::strong_actor_ptr> {
        inline size_t operator()(const nil::actor::strong_actor_ptr &ptr) const {
            return ptr ? static_cast<size_t>(ptr->id()) : 0;
        }
    };

    template<>
    struct hash<nil::actor::weak_actor_ptr> {
        inline size_t operator()(const nil::actor::weak_actor_ptr &ptr) const {
            return ptr ? static_cast<size_t>(ptr->id()) : 0;
        }
    };
