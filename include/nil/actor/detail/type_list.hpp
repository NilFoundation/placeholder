    ...
    namespace actor {
        namespace detail {
            ...
            struct tl_slice_impl<0, Remaining, PadType, List, Ts...> {
                using type =
                    typename tl_slice_impl<0, Remaining - 1, PadType, tl_tail_t<List>, Ts..., tl_head_t<List>>::type;
            };
            ...
            struct tl_zip {
                ...
                using type = typename tl_zip_impl<tl_slice_t<ListA, 0, result_size>, tl_slice_t<ListB, 0, result_size>,
                                                  Fun>::type;
            };
            ...
            struct tl_find_impl<Pred, T, Ts...> {
                using type =
                    typename std::conditional<Pred<T>::value, T, typename tl_find_impl<Pred, Ts...>::type>::type;
            };
            ...
            struct tl_count {
                static constexpr size_t value =
                    (Pred<tl_head_t<List>>::value ? 1 : 0) + tl_count<tl_tail_t<List>, Pred>::value;
            };
            ...
            struct tl_map_conditional {
                using type = tl_concat_t<type_list<typename std::conditional<Trait<tl_head_t<List>>::value == TRes,
            ...
            struct tl_distinct<type_list<T0, Ts...>> {
                using type =
                    tl_concat_t<type_list<T0>, typename tl_distinct<tl_filter_type_t<type_list<Ts...>, T0>>::type>;
            };
            ...
            struct tl_pad_right_impl<List, true, OldSize, NewSize, FillType> {
                using type = typename tl_pad_right_impl<tl_push_back_t<List, FillType>, OldSize + 1 < NewSize,
            ...
            struct tl_pad_right {
                using type = typename tl_pad_right_impl<List, (tl_size<List>::value < NewSize), tl_size<List>::value,
            ...
            struct tl_pad_left_impl {
                using type =
                    typename tl_pad_left_impl<tl_push_front_t<List, FillType>, OldSize + 1, NewSize, FillType>::type;
            };
            ...
            struct tl_pad_left {
                ...
                using type = typename tl_pad_left_impl<List, list_size, (list_size > NewSize) ? list_size : NewSize,
                                                       FillType>::type;
            };
            ...
            struct tl_intersect_impl<type_list<T0, Ts...>, false> {
                using type = tl_concat_t<type_list<T0>,
            ...
            struct tl_group_by_impl {
                ...
                using suffix = typename tl_group_by_impl_step<Pred<tl_head<In>, tl_back<last_group>>::value,
                                                              tl_head_t<In>, last_group>::type;
