            ...
            namespace parser {
                ...
                void read_ipv6_h16(state<Iterator, Sentinel> &ps, Consumer &consumer) {
                    ...
                    state(init) {transition(read, hexadecimal_chars, rd_hex(ch), pec::integer_overflow)} term_state(
                        read) {
                        transition_if(digits < 4, read, hexadecimal_chars, rd_hex(ch), pec::integer_overflow)} fin();
                }
                ...
                void read_ipv6_h16_or_l32(state<Iterator, Sentinel> &ps, Consumer &consumer) {
                    ...
                    start();
                    state(init) {transition(read, hexadecimal_chars, rd_both(ch), pec::integer_overflow)} term_state(
                        read) {
                        transition_if(mode == indeterminate, read, hexadecimal_chars, rd_both(ch),
                                      pec::integer_overflow)
                            transition_if(mode == v6_bits, read, hexadecimal_chars, rd_hex(ch), pec::integer_overflow)
                        ...
                        transition(read_last_octet, decimal_chars, rd_dec(ch), pec::integer_overflow)} fin();
                }
                ...
                void read_ipv6_address(state<Iterator, Sentinel> &ps, Consumer &&consumer) {
                    ...
                    state(init) {transition(rd_sep, ':') fsm_epsilon(read_ipv6_h16_or_l32(ps, prefix_consumer),
                    ...
                    unstable_state(maybe_has_l32) {epsilon_if(promote_v4(), done) epsilon(rd_prefix_sep)}
                    // Got ":" at a position where it can only be parsed as "::".
                    state(rd_sep) {transition(has_sep, ':')}    // Stops parsing after reading "::" (all-zero address)
                                                                // or proceeds with reading the suffix.
                    term_state(has_sep) {epsilon(rd_suffix, hexadecimal_chars)}
                    // Read part of the prefix, i.e., everything before "::".
                    state(rd_prefix) {
                        fsm_epsilon_if(remaining_bytes() > 4, read_ipv6_h16(ps, prefix_consumer), rd_prefix_sep,
                                       hexadecimal_chars)
                            fsm_epsilon_if(remaining_bytes() == 4, read_ipv6_h16_or_l32(ps, prefix_consumer),
                                           maybe_done, hexadecimal_chars)
                                fsm_epsilon_if(remaining_bytes() == 2, read_ipv6_h16(ps, prefix_consumer), done,
                                               hexadecimal_chars)}
                    ...
                    state(rd_suffix) {fsm_epsilon_if(remaining_bytes() >= 4, read_ipv6_h16_or_l32(ps, suffix_consumer),
