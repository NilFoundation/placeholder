...
#include <nil/actor/detail/make_unique.hpp>
...
namespace nil::actor::detail {
    ...
    class BOOST_SYMBOL_VISIBLE simple_actor_clock : public actor_clock {
    public:
        // -- member types -----------------------------------------------------------
        ...
        struct ordinary_timeout final : delayed_event {
            ...
            ordinary_timeout(time_point due, strong_actor_ptr self, std::string type, uint64_t id) :
                delayed_event(ordinary_timeout_type, due), self(std::move(self)), type(std::move(type)), id(id) {
                // nop
            }
        ...
        struct multi_timeout final : delayed_event {
            ...
            multi_timeout(time_point due, strong_actor_ptr self, std::string type, uint64_t id) :
                delayed_event(multi_timeout_type, due), self(std::move(self)), type(std::move(type)), id(id) {
                // nop
            }
        ...
        struct actor_msg final : delayed_event {
            ...
            actor_msg(time_point due, strong_actor_ptr receiver, mailbox_element_ptr content) :
                delayed_event(actor_msg_type, due), receiver(std::move(receiver)), content(std::move(content)) {
                // nop
            }
        ...
        struct group_msg final : delayed_event {
            ...
            group_msg(time_point due, group target, strong_actor_ptr sender, message content) :
                delayed_event(group_msg_type, due), target(std::move(target)), sender(std::move(sender)),
        ...
        struct multi_timeout_cancellation final : cancellation {
            multi_timeout_cancellation(actor_id aid, std::string type, uint64_t id) :
                cancellation(ordinary_timeout_cancellation_type, aid), type(std::move(type)), id(id) {
                // nop
            }
        ...
        struct timeouts_cancellation final : cancellation {
            timeouts_cancellation(actor_id aid) : cancellation(timeouts_cancellation_type, aid) {
                // nop
            }
        };
