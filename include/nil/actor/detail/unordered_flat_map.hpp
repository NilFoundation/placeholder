...
namespace nil::actor::detail {
    ...
    public:
        ...
        unordered_flat_map(InputIterator first, InputIterator last) : xs_(first, last) {
            // nop
        }
        ...
        iterator insert(const_iterator hint, value_type x) {
            auto i = find(x.first);
            return i == end() ? xs_.insert(gcc48_iterator_workaround(hint), std::move(x)) : i;
        }
        ...
        iterator erase(const_iterator first, const_iterator last) {
            return xs_.erase(gcc48_iterator_workaround(first), gcc48_iterator_workaround(last));
        }
        ...
        mapped_type &at(const K &key) {
            auto i = find(key);
            if (i == end())
                ACTOR_RAISE_ERROR(std::out_of_range, "nil::actor::detail::unordered_flat_map::at out of range");
            return i->second;
        }
    ...
    private:
        ...
        struct is_valid_erase_iter {
            template<class U>
            static auto sfinae(U *x) -> decltype(std::declval<vector_type &>().erase(*x), std::true_type {});
