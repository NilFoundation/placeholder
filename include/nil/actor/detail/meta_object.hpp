...
namespace nil {
    namespace actor {
        namespace detail {
            ...
            BOOST_SYMBOL_VISIBLE nil::actor::error save(const meta_object &meta, nil::actor::serializer &sink,
                                                        const void *obj);

            /// Convenience function for calling `meta.save_binary(sink, obj)`.
            BOOST_SYMBOL_VISIBLE nil::actor::error_code<sec> save(const meta_object &meta,
                                                                  nil::actor::binary_serializer &sink, const void *obj);

            /// Convenience function for calling `meta.load(source, obj)`.
            BOOST_SYMBOL_VISIBLE nil::actor::error load(const meta_object &meta, nil::actor::deserializer &source,
                                                        void *obj);

            /// Convenience function for calling `meta.load_binary(source, obj)`.
            BOOST_SYMBOL_VISIBLE nil::actor::error_code<sec> load(const meta_object &meta,
                                                                  nil::actor::binary_deserializer &source, void *obj);
