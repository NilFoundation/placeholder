    ...
    namespace actor {
        namespace detail {
            ...
            void profiled_send(Self *self, SelfHandle &&src, const Handle &dst, message_id msg_id,
                ...
                if (dst) {
                    auto element = make_mailbox_element(std::forward<SelfHandle>(src), msg_id, std::move(stages),
                                                        std::forward<Ts>(xs)...);
            ...
            void profiled_send(Self *self, SelfHandle &&src, const Handle &dst, actor_clock &clock,
                ...
                if (dst) {
                    if constexpr (std::is_same<Handle, group>::value) {
                        ...
                        auto element = make_mailbox_element(std::forward<SelfHandle>(src), msg_id, no_stages,
                                                            std::forward<Ts>(xs)...);
