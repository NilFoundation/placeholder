...
namespace nil::actor::detail {
    ...
    public:
        ...
        unique_function(unique_function &&other) : holds_wrapper_(other.holds_wrapper_) {
            fptr_ = other.fptr_;
        ...
        explicit unique_function(raw_pointer fun) : holds_wrapper_(false), fptr_(fun) {
            // nop
        }

        explicit unique_function(wrapper_pointer ptr) : holds_wrapper_(true), wptr_(ptr) {
            // nop
        }

        template<class T, class = typename std::enable_if<
                              !std::is_convertible<T, raw_pointer>::value &&
                              std::is_same<decltype((std::declval<T &>())(std::declval<Ts>()...)), R>::value>::type>
