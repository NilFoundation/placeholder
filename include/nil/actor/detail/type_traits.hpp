...
namespace nil {
    namespace actor {
        namespace detail {
            ...
            template<class T>
            struct is_builtin {
                static constexpr bool value = std::is_arithmetic<T>::value || is_duration<T>::value ||
                                              is_one_of<T, timestamp, std::string, std::u16string, std::u32string,
                                                        atom_value, message, actor, group, node_id>::value;
            };
            ...
            struct is_primitive {
                static constexpr bool value =
                    std::is_arithmetic<T>::value || std::is_convertible<T, std::string>::value ||
                    std::is_convertible<T, std::u16string>::value || std::is_convertible<T, std::u32string>::value ||
...
template <class T, class U,
          bool Enable = std::is_integral<T>::value
            ...
            private:
                template<class U>
                static auto sfinae(U *x) -> decltype(std::declval<To *>() = x->data(), std::true_type());
