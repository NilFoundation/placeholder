...
#define ACTOR_HAS_MEMBER_TRAIT(name)                                         \
    template<class T>                                                      \
...
#define ACTOR_HAS_ALIAS_TRAIT(name)                         \
    template<class T>                                     \
...
namespace nil::actor::detail {
    ...
    struct is_primitive {
        static constexpr bool value = std::is_convertible<T, std::string>::value ||
                                      ...
                                      std::is_convertible<T, std::u32string>::value || std::is_arithmetic<T>::value;
    };
    ...
    class is_comparable {
        ...
        static bool cmp_help_fun(const A *arg0, const B *arg1, decltype(*arg0 == *arg1) *,
                                 std::integral_constant<bool, false>);

        // silences float-equal warnings caused by decltype(*arg0 == *arg1)
        template<class A, class B>
        static bool cmp_help_fun(const A *, const B *, bool *, std::integral_constant<bool, true>);

        template<class A, class B, class C>
        static void cmp_help_fun(const A *, const B *, void *, C);

        using result_type = decltype(cmp_help_fun(
            static_cast<T1 *>(nullptr), static_cast<T2 *>(nullptr), static_cast<bool *>(nullptr),
            std::integral_constant<bool, std::is_arithmetic<T1>::value && std::is_arithmetic<T2>::value> {}));
    ...
    public:
        static constexpr bool value = is_primitive<T>::value == false && std::is_same<bool, result_type>::value;
    };
    ...
    struct callable_trait<R(Ts...)> {
        ...
        using message_view_type =
            std::conditional_t<has_param_args, param_message_view<param_decay_t<Ts>...>,
    ...
    struct is_manipulator {
        static constexpr bool value = tl_exists<typename get_callable_trait<F>::arg_types, is_mutable_ref>::value;
    };
    ...
    private:
        template<class U>
        static int fun(const U *, decltype(std::declval<U &>().peek_all(unit)) * = nullptr);
    ...
    ACTOR_HAS_MEMBER_TRAIT(size);
    ACTOR_HAS_MEMBER_TRAIT(data);
    ACTOR_HAS_MEMBER_TRAIT(make_behavior);

    /// Checks whether F is convertible to either `std::function<void (T&)>`
    /// or `std::function<void (const T&)>`.
    template<class F, class T>
    struct is_handler_for {
        static constexpr bool value = std::is_convertible<F, std::function<void(T &)>>::value ||
    ...
    template<class F, class T>
    using is_handler_for_ef = typename std::enable_if<is_handler_for<F, T>::value>::type;
...
template <class T, class U,
    ...
    struct is_equal_int_type {
        static constexpr bool value = sizeof(T) == sizeof(U) && std::is_signed<T>::value == std::is_signed<U>::value;
    };
    ...
    template<class T, typename U>
    struct is_same_ish : std::conditional<std::is_same<T, U>::value, std::true_type, is_equal_int_type<T, U>>::type {};
    ...
    ACTOR_HAS_ALIAS_TRAIT(value_type);

    ACTOR_HAS_ALIAS_TRAIT(key_type);

    ACTOR_HAS_ALIAS_TRAIT(mapped_type);
    ...
    struct all_constructible<type_list<T, Ts...>, type_list<U, Us...>> {
        static constexpr bool value =
            std::is_constructible<T, U>::value && all_constructible<type_list<Ts...>, type_list<Us...>>::value;
    };
    ...
    struct is_map_like {
        static constexpr bool value =
            is_iterable<T>::value && has_key_type_alias<T>::value && has_mapped_type_alias<T>::value;
    };
    ...
    struct is_list_like {
        static constexpr bool value =
            is_iterable<T>::value && has_value_type_alias<T>::value && !has_mapped_type_alias<T>::value;
    };
    ...
    private:
        template<class U>
        static auto sfinae(U *f) -> decltype((*f)(std::declval<Ts>()...), std::true_type());
    ...
    private:
        template<class U>
        static auto sfinae(U *f) -> std::is_same<R, decltype((*f)(std::declval<Ts>()...))>;
    ...
    private:
        template<class U>
        static auto sfinae(U *x) -> std::integral_constant<bool, std::is_convertible<decltype(x->data()), To *>::value>;
    ...
    struct can_apply {
        template<class U>
        static auto sfinae(U *x) -> decltype(x->apply(std::declval<Arg>()), std::true_type {});
    ...
    struct is_stl_tuple_type {
        template<class U>
        static auto sfinae(U *) -> decltype(std::integral_constant<bool, std::tuple_size<U>::value >= 0> {});
    ...
    template<class T>
    constexpr bool is_complete = decltype(is_complete_impl(std::declval<T *>()))::value;

}    // namespace nil::actor::detail


#undef ACTOR_HAS_MEMBER_TRAIT
#undef ACTOR_HAS_ALIAS_TRAIT