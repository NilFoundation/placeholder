...
#include <nil/actor/core/internal/io_desc.hh>
#include <sys/types.h>
...
namespace seastar {
    namespace internal {
        ...
        private:
            ...
            explicit io_request(operation op, int fd, int flags, ::msghdr *msg) : _op(op), _fd(fd) {
                _attr.flags = flags;
                _ptr.msghdr = msg;
            }

            explicit io_request(operation op, int fd, sockaddr *sa, socklen_t sl) : _op(op), _fd(fd) {
                _ptr.sockaddr = sa;
                _size.socklen = sl;
            }

            explicit io_request(operation op, int fd, int flags, sockaddr *sa, socklen_t *sl) : _op(op), _fd(fd) {
                _attr.flags = flags;
                _ptr.sockaddr = sa;
                _size.socklen_ptr = sl;
            }
            explicit io_request(operation op, int fd, uint64_t pos, char *ptr, size_t size) : _op(op), _fd(fd) {
                _attr.pos = pos;
                _ptr.addr = ptr;
                _size.len = size;
            }

            explicit io_request(operation op, int fd, uint64_t pos, iovec *ptr, size_t size) : _op(op), _fd(fd) {
                _attr.pos = pos;
                _ptr.iovec = ptr;
                _size.len = size;
            }

            explicit io_request(operation op, int fd) : _op(op), _fd(fd) {
            }
            explicit io_request(operation op, int fd, int events) : _op(op), _fd(fd) {
                _attr.events = events;
            }

            explicit io_request(operation op, int fd, char *ptr) : _op(op), _fd(fd) {
                _ptr.addr = ptr;
            }
        ...
        public:
            ...
            static io_request make_send(int fd, const void *address, size_t size, int flags) {
                return io_request(operation::send, fd, flags,
                                  const_cast<char *>(reinterpret_cast<const char *>(address)), size);
            }
            ...
            static io_request make_write(int fd, uint64_t pos, const void *address, size_t size) {
                return io_request(operation::write, fd, pos,
                                  const_cast<char *>(reinterpret_cast<const char *>(address)), size);
            }
