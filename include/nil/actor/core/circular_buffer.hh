...
namespace seastar {
    ...
    class circular_buffer {
        ...
        static_assert(!std::is_default_constructible_v<Alloc> || std::is_nothrow_default_constructible_v<Alloc>);
    ...
    template<typename T, typename Alloc>
    inline void circular_buffer<T, Alloc>::reserve(size_t size) {
        if (capacity() < size) {
    ...
    template<typename T, typename Alloc>
    inline void circular_buffer<T, Alloc>::clear() {
        erase(begin(), end());
    }

    template<typename T, typename Alloc>
    inline circular_buffer<T, Alloc>::circular_buffer(Alloc alloc) noexcept : _impl(std::move(alloc)) {
    }

    template<typename T, typename Alloc>
    inline circular_buffer<T, Alloc>::circular_buffer(circular_buffer &&x) noexcept : _impl(std::move(x._impl)) {
        x._impl.reset();
    }

    template<typename T, typename Alloc>
    inline circular_buffer<T, Alloc> &circular_buffer<T, Alloc>::operator=(circular_buffer &&x) noexcept {
        if (this != &x) {
    ...
    inline circular_buffer<T, Alloc>::~circular_buffer() {
        for_each([this](T &obj) { std::allocator_traits<Alloc>::destroy(_impl, &obj); });
    ...
    template<typename T, typename Alloc>
    void circular_buffer<T, Alloc>::expand(size_t new_cap) {
    ...
    template<typename T, typename Alloc>
    inline T &circular_buffer<T, Alloc>::front() noexcept {
        return _impl.storage[mask(_impl.begin)];
    }

    template<typename T, typename Alloc>
    inline const T &circular_buffer<T, Alloc>::front() const noexcept {
        return _impl.storage[mask(_impl.begin)];
    }

    template<typename T, typename Alloc>
    inline T &circular_buffer<T, Alloc>::back() noexcept {
        return _impl.storage[mask(_impl.end - 1)];
    }

    template<typename T, typename Alloc>
    inline const T &circular_buffer<T, Alloc>::back() const noexcept {
        return _impl.storage[mask(_impl.end - 1)];
    }

    template<typename T, typename Alloc>
    inline void circular_buffer<T, Alloc>::pop_front() noexcept {
        std::allocator_traits<Alloc>::destroy(_impl, &front());
        ++_impl.begin;
    }

    template<typename T, typename Alloc>
    inline void circular_buffer<T, Alloc>::pop_back() noexcept {
        std::allocator_traits<Alloc>::destroy(_impl, &back());
        --_impl.end;
    }

    template<typename T, typename Alloc>
    inline T &circular_buffer<T, Alloc>::operator[](size_t idx) noexcept {
        return _impl.storage[mask(_impl.begin + idx)];
    }

    template<typename T, typename Alloc>
    inline const T &circular_buffer<T, Alloc>::operator[](size_t idx) const noexcept {
        return _impl.storage[mask(_impl.begin + idx)];
    }

    template<typename T, typename Alloc>
    inline T &circular_buffer<T, Alloc>::access_element_unsafe(size_t idx) noexcept {
        return _impl.storage[mask(_impl.begin + idx)];
    }

    template<typename T, typename Alloc>
    inline typename circular_buffer<T, Alloc>::iterator circular_buffer<T, Alloc>::erase(iterator first,
