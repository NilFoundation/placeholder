...
namespace seastar {
            ...
            future_type get_future(time_point timeout = time_point::max()) noexcept {
                ...
                if (!_original_future.available()) {
                    ...
                    if (_original_future._state.valid()) {
                        ...
                        (void)_original_future.then_wrapped(
                            [s = this->shared_from_this()](future_type &&f) mutable { s->resolve(std::move(f)); });
                    }
                    _peers.push_back(std::move(p), timeout);
                    return f;
                } else if (_original_future.failed()) {
                    return future_type(exception_future_marker(),
                                       std::exception_ptr(_original_future._state.get_exception()));
                } else {
    ...
    public:
        ...
        shared_future(future_type f) : _state(make_lw_shared<shared_state>(std::move(f))) {
        }

        shared_future() = default;    // noexcept, based on the respective lw_shared_ptr constructor
        shared_future(const shared_future &) =
            default;    // noexcept, based on the respective lw_shared_ptr constructor
    ...
    public:
        ...
        shared_promise(shared_promise &&) =
            default;    // noexcept, based on the respective promise and shared_future constructors
        shared_promise &operator=(shared_promise &&) =
            default;    // noexcept, based on the respective promise and shared_future constructors
