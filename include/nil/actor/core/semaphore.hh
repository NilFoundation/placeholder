...
namespace seastar {
    ...
    public:
        ...
        basic_semaphore(size_t count) noexcept(std::is_nothrow_default_constructible_v<exception_factory>) :
            exception_factory(), _count(count), _wait_list(make_expiry_handler()) {
        }
        basic_semaphore(size_t count,
                        exception_factory &&factory) noexcept(std::is_nothrow_move_constructible_v<exception_factory>) :
            exception_factory(std::move(factory)),
            _count(count), _wait_list(make_expiry_handler()) {
    ...
    inline void basic_semaphore<ExceptionFactory, Clock>::broken(std::exception_ptr xp) noexcept {
    ...
    public:
        ...
        /// Return ownership of some units to the semaphore. The semaphore will be signaled by the number of units
        /// returned.
        ///
    ...
    /// \related semaphore
    template<typename ExceptionFactory, typename Clock = typename timer<>::clock>
    future<semaphore_units<ExceptionFactory, Clock>> get_units(basic_semaphore<ExceptionFactory, Clock> &sem,
                                                               size_t units) noexcept {
        return sem.wait(units).then([&sem, units] { return semaphore_units<ExceptionFactory, Clock> {sem, units}; });
    }
    ...
    /// \brief Take units from semaphore temporarily with time bound on wait
    ///
    /// Like \ref get_units(basic_semaphore<ExceptionFactory>&, size_t, basic_semaphore<ExceptionFactory>::time_point)
    /// but allow the timeout to be specified as a duration.
    ///
    /// \param sem The semaphore to take units from
    /// \param units  Number of units to take
    ...
    template<typename ExceptionFactory, typename Clock = typename timer<>::clock>
    semaphore_units<ExceptionFactory, Clock> consume_units(basic_semaphore<ExceptionFactory, Clock> &sem,
                                                           size_t units) noexcept {
        sem.consume(units);
        return semaphore_units<ExceptionFactory, Clock> {sem, units};
    }
    ...
    inline futurize_t<std::result_of_t<Func()>> with_semaphore(basic_semaphore<ExceptionFactory, Clock> &sem,
    ...
    inline futurize_t<std::result_of_t<Func()>>
        with_semaphore(basic_semaphore<ExceptionFactory, Clock> &sem, size_t units,
