...
namespace seastar {
    ...
    public:
        ...
        /// Moves a deleter.
        deleter(deleter &&x) noexcept : _impl(x._impl) {
            x._impl = nullptr;
        }
        /// \cond internal
        explicit deleter(impl *i) noexcept : _impl(i) {
        }
        deleter(raw_object_tag tag, void *object) noexcept : _impl(from_raw_object(object)) {
        }
        /// \endcond
    ...
    inline deleter &deleter::operator=(deleter &&x) noexcept {
        if (this != &x) {
            this->~deleter();
    ...
    struct lambda_deleter_impl final : deleter::impl {
        Deleter del;
        lambda_deleter_impl(deleter next, Deleter &&del) : impl(std::move(next)), del(std::move(del)) {
        }
        virtual ~lambda_deleter_impl() override {
            del();
        }
    };
    ...
    struct object_deleter_impl final : deleter::impl {
        Object obj;
        object_deleter_impl(deleter next, Object &&obj) : impl(std::move(next)), obj(std::move(obj)) {
        }
    };

    template<typename Object>
    inline object_deleter_impl<Object> *make_object_deleter_impl(deleter next, Object obj) {
    ...
    struct free_deleter_impl final : deleter::impl {
        ...
        virtual ~free_deleter_impl() override {
            std::free(obj);
        }
    };
    /// \endcond
