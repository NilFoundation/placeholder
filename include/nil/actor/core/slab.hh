...
namespace seastar {
    ...
    public:
        slab_page_desc(void *slab_page, size_t objects, size_t object_size, uint8_t slab_class_id, uint32_t index) :
            _slab_page(slab_page), _refcnt(0U), _index(index), _magic(SLAB_MAGIC_NUMBER),
    ...
    private:
        ...
        inline Item *create_item(void *object, uint32_t slab_page_index, Args &&...args) {
        ...
        inline std::pair<void *, uint32_t> evict_lru_item(std::function<void(Item &item_ref)> &erase_func) {
            if (_lru.empty()) {
                return {nullptr, 0U};
            }
    ...
    public:
        slab_class(size_t size, uint8_t slab_class_id) : _size(size), _slab_class_id(slab_class_id) {
        }
        ...
        Item *create_from_new_page(uint64_t max_object_size, uint32_t slab_page_index,
                                   std::function<void(slab_page_desc &desc)> insert_slab_page_desc, Args &&...args) {
            // allocate slab page.
    ...
    private:
        ...
        void register_metrics() {
            ...
            _metrics.add_group(
                "slab",
                {sm::make_derive("malloc_total_operations", sm::description("Total number of slab malloc operations"),
                                 _stats.allocs),
                 sm::make_derive("free_total_operations", sm::description("Total number of slab free operations"),
                                 _stats.frees),
                 sm::make_gauge("malloc_objects", sm::description("Number of slab created objects currently in memory"),
                                [this] { return _stats.allocs - _stats.frees; })});
        }
        ...
        inline bool can_allocate_page(slab_class<Item> &sc) {
            return (_reclaimer && !_reclaimed) || (_available_slab_pages > 0 || sc.has_no_slab_pages());
        }

    public:
        slab_allocator(double growth_factor, uint64_t limit, uint64_t max_object_size) :
            _max_object_size(max_object_size), _available_slab_pages(limit / max_object_size) {
        ...
        Item *create(const size_t size, Args &&...args) {
            ...
            if (!slab_class->empty()) {
                ...
                if (can_allocate_page(*slab_class)) {
                    ...
                    item = slab_class->create_from_new_page(
                        _max_object_size, index_to_insert,
