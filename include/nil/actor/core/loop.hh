...
namespace seastar {
    ...
    template<typename AsyncAction>
    SEASTAR_CONCEPT(requires seastar::InvokeReturns<AsyncAction, stop_iteration> ||
                    seastar::InvokeReturns<AsyncAction, future<stop_iteration>>)
    ...
    namespace internal {
        ...
        public:
            ...
            repeat_until_value_state(std::optional<T> st, AsyncAction action) :
                repeat_until_value_state(std::move(action)) {
    ...
    namespace internal {
        ...
        public:
            explicit do_until_state(StopCondition stop, AsyncAction action) :
                _stop(std::move(stop)), _action(std::move(action)) {
            }
            future<> get_future() {
                return _promise.get_future();
            }
    ...
    SEASTAR_CONCEPT(
        requires seastar::InvokeReturns<StopCondition, bool> &&seastar::InvokeReturns<AsyncAction, future<>>)
    ...
    namespace internal {
        ...
        template<typename Iterator, typename AsyncAction>
        inline future<> do_for_each_impl(Iterator begin, Iterator end, AsyncAction action) {
            while (begin != end) {
                ...
                if (!f.available() || need_preempt()) {
                    auto *s = new internal::do_for_each_state<Iterator, AsyncAction> {std::move(begin), std::move(end),
                                                                                      std::move(action), std::move(f)};
                    return s->get_future();
                }
            }
            return make_ready_future<>();
        ...
        while (begin != end) {
            ...
            if (!f.available() || f.failed()) {
                if (!s) {
                    ...
                    auto n = (internal::iterator_range_estimate_vector_capacity(begin, end,
    ...
    inline future<> max_concurrent_for_each(Iterator begin, Iterator end, size_t max_concurrent, Func &&func) noexcept {
        struct state {
            Iterator begin;
            Iterator end;
            Func func;
            ...
            state(Iterator begin_, Iterator end_, size_t max_concurrent_, Func func_) :
                begin(std::move(begin_)), end(std::move(end_)), func(std::move(func_)), max_concurrent(max_concurrent_),
                sem(max_concurrent_), err() {
            }
        };
        ...
        try {
            return do_with(
                state(std::move(begin), std::move(end), max_concurrent, std::forward<Func>(func)), [](state &s) {
                    return do_until([&s] { return s.begin == s.end; },
                                    [&s] {
