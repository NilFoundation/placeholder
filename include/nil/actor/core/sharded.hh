...
namespace seastar {
    ...
    public:
        ...
        inline future<Initial> map_reduce0(Mapper map, Initial initial, Reduce reduce) {
            ...
            return ::seastar::map_reduce(smp::all_cpus().begin(), smp::all_cpus().end(), std::move(wrapped_map),
        ...
        Ret invoke_on(unsigned id, smp_submit_to_options options, Func &&func, Args &&...args) {
            return smp::submit_to(
                id, options, [this, func = std::forward<Func>(func), args = std::tuple(std::move(args)...)]() mutable {
                    auto inst = get_local_service();
    ...
    template<typename Func, typename... Args>
    SEASTAR_CONCEPT(requires std::invocable<Func, Service &, Args...>)
    inline future<> sharded<Service>::invoke_on_all(smp_submit_to_options options, Func func, Args... args) noexcept {
        ...
        try {
            return invoke_on_all(options, invoke_on_all_func_type([func, args = std::tuple(std::move(args)...)](
                                 ...
                                 [orig = this_shard_id(), func = std::move(func),
                                  args = std::tuple(std::move(args)...)](Service &s) -> future<> {
                                     return this_shard_id() == orig ?
                                                make_ready_future<>() :
                                                futurize_apply(func, std::tuple_cat(std::forward_as_tuple(s), args));
                                     ;
                                 });
        } catch (...) {
            return current_exception_as_future();
