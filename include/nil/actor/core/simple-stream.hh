...
namespace seastar {
    ...
    public:
        ...
        [[gnu::always_inline]] void fill(char c, size_t size) {
            if (size > _size) {
                throw std::out_of_range("serialization buffer overflow");
            }
        ...
        [[gnu::always_inline]] const size_t size() const {
            return _size;
        }
    ...
    public:
        ...
        void fill(char c, size_t size) {
            for_each_fragment(size, [c](simple fragment) { std::fill_n(fragment.begin(), fragment.size(), c); });
        }
    ...
    public:
        ...
        using iterator_type = Iterator;
        memory_output_stream() : _is_simple(true), _simple() {
        }
        ...
        [[gnu::always_inline]] memory_output_stream(memory_output_stream &&other) noexcept :
            _is_simple(other._is_simple) {
            if (_is_simple) {
        ...
        [[gnu::always_inline]] void fill(char c, size_t size) {
            with_stream([c, size](auto &stream) { stream.fill(c, size); });
        }
    ...
    private:
        ...
        fragmented_memory_input_stream(Iterator it, simple bv, size_t size) : _it(it), _current(bv), _size(size) {
        }
    ...
    public:
        using has_with_stream = std::false_type;
        using iterator_type = Iterator;
        fragmented_memory_input_stream(Iterator it, size_t size) : _it(it), _size(size) {
        }
    ...
    */
    ...
    public:
        ...
        [[gnu::always_inline]] memory_input_stream(const memory_input_stream &other) noexcept :
            _is_simple(other._is_simple) {
            // Making this copy constructor noexcept makes copy assignment simpler.
        ...
        [[gnu::always_inline]] memory_input_stream(memory_input_stream &&other) noexcept :
            _is_simple(other._is_simple) {
            if (_is_simple) {
                new (&_simple) simple(std::move(other._simple));
            } else {
    ...
    [[gnu::always_inline]] static inline decltype(auto) with_serialized_stream(Stream &stream,
