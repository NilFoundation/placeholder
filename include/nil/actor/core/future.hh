...
namespace seastar {
    ...
    namespace internal {
        ...
        // It doesn't seem to be possible to use std::tuple_element_t with an empty tuple. There is an static_assert in
        // it that fails the build even if it is in the non enabled side of std::conditional.
        template<typename T>
        ...
        ///
        /// The advantage over just using a union directly is that this uses inheritance when possible and so benefits
        ...
        // The objective is to avoid extra space for empty types like std::tuple<>. We could use std::is_empty_v, but it
        // is better to check that both the constructor and destructor can be skipped.
        template<typename T>
    ...
    private:
        ...
        future(future_state_base::nested_exception_marker m, future_state_base &&n, future_state_base &&old) noexcept :
    ...
    public:
        ...
        future<T SEASTAR_ELLIPSIS> handle_exception(Func &&func) noexcept {
            return then_wrapped([func = std::forward<Func>(func)](auto &&fut) mutable -> future<T SEASTAR_ELLIPSIS> {
                if (!fut.failed()) {
