    ...
    namespace actor {
        ...
        public:
            ...
            void set_filter(stream_slot slot, filter_type new_filter) {
                ACTOR_LOG_TRACE(ACTOR_ARG(slot) << ACTOR_ARG(new_filter));
            ...
            filter_type &filter(stream_slot slot) {
                ...
                ACTOR_RAISE_ERROR("invalid slot");
            }
            ...
            bool all_filters(UnaryPredicate predicate) {
                return std::all_of(
                    ...
                    [&](const typename state_map_type::value_type &kvp) { return predicate(kvp.second.filter); });
            }
            ...
            bool insert_path(unique_path_ptr ptr) override {
                ...
                if (!super::insert_path(std::move(ptr))) {
                    ACTOR_LOG_DEBUG("unable to insert path at slot" << slot);
                    return false;
                }
                // Append to the state map.
                if (!state_map_.emplace(slot, path_state {}).second) {
                    ACTOR_LOG_DEBUG("unable to add state for slot" << slot);
            ...
            void emit_batches() override {
                ACTOR_LOG_TRACE(ACTOR_ARG2("buffered", this->buffered()) << ACTOR_ARG2("paths", this->paths_.size()));
            ...
            void fan_out_flush() {
                auto &buf = this->buf_;
                auto f = [&](typename map_type::value_type &x, typename state_map_type::value_type &y) {
        ...
        private:
            void emit_batches_impl(bool force_underfull) {
                ...
                // caches at the most.
                auto not_closing = [&](typename map_type::value_type &x, typename state_map_type::value_type &) {
                    return !x.second->closing;
                };
                // Returns the minimum of `interim` and `get_credit(x) - get_cache_size(y)`.
                auto f = [&](size_t interim, typename map_type::value_type &x, typename state_map_type::value_type &y) {
                ...
                auto chunk_size = detail::zip_fold_if(f, not_closing, std::numeric_limits<size_t>::max(),
                ...
                if (chunk_size == std::numeric_limits<size_t>::max()) {
                    ...
                    auto g = [&](typename map_type::value_type &x, typename state_map_type::value_type &y) {
                ...
                if (chunk.empty()) {
                    auto g = [&](typename map_type::value_type &x, typename state_map_type::value_type &y) {
                        // Always force batches on closing paths.
                        x.second->emit_batches(this->self(), y.second.buf, force_underfull || x.second->closing);
                    };
                    detail::zip_foreach(g, this->paths_.container(), state_map_.container());
                } else {
                    auto g = [&](typename map_type::value_type &x, typename state_map_type::value_type &y) {
                        ...
                        // Always force batches on closing paths.
                        x.second->emit_batches(this->self(), st.buf, force_underfull || x.second->closing);
                    };
                    detail::zip_foreach(g, this->paths_.container(), state_map_.container());
                }
            }
