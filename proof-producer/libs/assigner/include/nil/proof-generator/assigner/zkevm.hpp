...
namespace nil {
    namespace proof_generator {
        ...
        std::optional<std::string> fill_zkevm_assignment_table(nil::crypto3::zk::snark::plonk_assignment_table<BlueprintFieldType>& assignment_table,
            ...
            auto rw_operations = deserialize_rw_traces_from_file(rw_trace_path, options.get_base_index(contract_bytecodes->index));
            if (!rw_operations) {
            ...
            const auto zkevm_states = deserialize_zkevm_state_traces_from_file(zkevm_trace_path, options.get_base_index(contract_bytecodes->index));
            if (!zkevm_states) {
            ...
            const auto copy_events = deserialize_copy_events_from_file(copy_trace_path, options.get_base_index(contract_bytecodes->index));
            if (!copy_events) {
            ...
            // const auto exp_operations = deserialize_exp_traces_from_file(exp_trace_path, options.get_base_index(contract_bytecodes->index));
            // if (!exp_operations) {
