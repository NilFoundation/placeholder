...
#include <nil/actor/detail/complexity_based_credit_controller.hpp>
#include <nil/actor/detail/meta_object.hpp>
#include <nil/actor/detail/size_based_credit_controller.hpp>
#include <nil/actor/detail/test_credit_controller.hpp>
...
namespace nil {
    namespace actor {
        ...
        namespace {
            ...
            void emit_ack_batch(inbound_path &path, credit_controller::assignment x, bool force_ack_msg = false) {
                ACTOR_ASSERT(x.batch_size > 0);
                ...
                auto guard = detail::make_scope_guard([&] {
                    ...
                    unsafe_send_as(path.self(), path.hdl,
                                   make<upstream_msg::ack_batch>(path.slots.invert(), path.self()->address(), 0,
                ...
                guard.disable();
                unsafe_send_as(path.self(), path.hdl,
                               make<upstream_msg::ack_batch>(path.slots.invert(), path.self()->address(), new_credit,
                                                             x.batch_size, path.last_batch_id, x.credit));
                path.last_acked_batch_id = path.last_batch_id;
        ...
        inbound_path::inbound_path(stream_manager_ptr mgr_ptr, stream_slots id, strong_actor_ptr ptr,
            ...
            hdl(std::move(ptr)), slots(id) {
            mgr->register_input_path(this);
            ACTOR_STREAM_LOG_DEBUG(mgr->self()->name() << "opens input stream with element type"
                                                     << detail::global_meta_object(in_type)->type_name << "at slot"
                                                     << id.receiver << "from" << hdl);
            if (auto str = get_if<std::string>(&self()->system().config(), "stream.credit-policy")) {
                if (*str == "testing")
        ...
        void inbound_path::handle(downstream_msg::batch &x) {
            ...
            if (assigned_credit <= batch_size) {
                ...
                ACTOR_STREAM_LOG_DEBUG_IF(next_credit_decision.time_since_epoch().count() > 0,
                                        mgr->self()->name() << "ran out of credit at slot" << slots.receiver);
            } else {
            ...
            if (threshold >= 0 && assigned_credit <= threshold)
                nil::actor::emit_ack_batch(*this, controller_->compute_bridge());
        ...
        void inbound_path::emit_ack_batch(local_actor *, int32_t, actor_clock::time_point now, timespan cycle) {
            ACTOR_LOG_TRACE(ACTOR_ARG(slots) << ACTOR_ARG(cycle));
            last_credit_decision = now;
            next_credit_decision = now + cycle;
            auto max_capacity = static_cast<int32_t>(mgr->out().max_capacity());
            nil::actor::emit_ack_batch(*this, controller_->compute(cycle, max_capacity), force_ack);
        }
