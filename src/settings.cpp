    ...
    namespace actor {
        ...
        const config_value *get_if(const settings *xs, string_view name) {
            ...
            if (i == xs->end() || !holds_alternative<config_value::dictionary>(i->second))
                return nullptr;
            return get_if(&get<config_value::dictionary>(i->second), name.substr(pos + 1));
        }
        ...
        config_value &put_impl(settings &dict, const std::vector<string_view> &path, config_value &value) {
            // Sanity check.
