...
namespace nil {
    namespace actor {
        ...
        spawner_config::spawner_config() :
            cli_helptext_printed(false), slave_mode(false), config_file_path(default_config_file),
            ...
            opt_group {custom_options_, "stream"}
                .add<timespan>(stream_desired_batch_complexity, "desired-batch-complexity", "processing time per batch")
                .add<timespan>(stream_max_batch_delay, "max-batch-delay", "maximum delay for partial batches")
                .add<timespan>(stream_credit_round_interval, "credit-round-interval", "time between emitting credit")
                .add<std::string>("credit-policy", "selects an algorithm for credit computation");
            ...
            opt_group(custom_options_, "work-stealing")
                ...
                .add<size_t>("aggressive-steal-interval", "frequency of aggressive steal attempts")
                .add<size_t>("moderate-poll-attempts", "nr. of moderate steal attempts")
                .add<size_t>("moderate-steal-interval", "frequency of moderate steal attempts")
                .add<timespan>("moderate-sleep-duration", "sleep duration between moderate steal attempts")
                .add<size_t>("relaxed-steal-interval", "frequency of relaxed steal attempts")
                .add<timespan>("relaxed-sleep-duration", "sleep duration between relaxed steal attempts");
            opt_group {custom_options_, "logger"}
                ...
                .add<std::string>("console", "std::clog output: none, colored, or uncolored")
            ...
            opt_group(custom_options_, "openssl")
                ...
                .add<string>(openssl_passphrase, "passphrase", "passphrase to decrypt the private key")
                .add<string>(openssl_capath, "capath", "path to an OpenSSL-style directory of trusted certificates")
                .add<string>(openssl_cafile, "cafile", "path to a file of concatenated PEM-formatted certificates");
        ...
        settings spawner_config::dump_content() const {
            settings result = content;
            // -- streaming parameters
            auto &stream_group = result["stream"].as_dictionary();
            put_missing(stream_group, "desired-batch-complexity", defaults::stream::desired_batch_complexity);
            put_missing(stream_group, "max-batch-delay", defaults::stream::max_batch_delay);
            put_missing(stream_group, "credit-round-interval", defaults::stream::credit_round_interval);
            put_missing(stream_group, "credit-policy", defaults::stream::credit_policy);
            put_missing(stream_group, "size-policy.buffer-capacity", defaults::stream::size_policy::buffer_capacity);
            put_missing(stream_group, "size-policy.bytes-per-batch", defaults::stream::size_policy::bytes_per_batch);
            ...
            put_missing(scheduler_group, "max-throughput", defaults::scheduler::max_throughput);
            put_missing(scheduler_group, "enable-profiling", false);
            put_missing(scheduler_group, "profiling-resolution", defaults::scheduler::profiling_resolution);
            ...
            put_missing(work_stealing_group, "moderate-poll-attempts", defaults::work_stealing::moderate_poll_attempts);
            ...
            put_missing(work_stealing_group, "relaxed-steal-interval", defaults::work_stealing::relaxed_steal_interval);
            put_missing(work_stealing_group, "relaxed-sleep-duration", defaults::work_stealing::relaxed_sleep_duration);
            // -- logger parameters
        ...
        error spawner_config::parse(int argc, char **argv, const char *ini_file_cstr) {
            string_list args;
            if (argc > 1)
        ...
        error spawner_config::parse(int argc, char **argv, std::istream &ini) {
            string_list args;
            if (argc > 1)
                args.assign(argv + 1, argv + argc);
        ...
        error spawner_config::parse(string_list args, std::istream &ini) {
            ...
            if (res.second != args.end()) {
                ...
                remainder.insert(remainder.end(), make_move_iterator(first), make_move_iterator(args.end()));
            } else {
                cli_helptext_printed = get_or(content, "help", false) || get_or(content, "long-help", false);
            }
        ...
        error spawner_config::parse(string_list args, const char *ini_file_cstr) {
        ...
        spawner_config &spawner_config::add_error_category(uint8_t x, error_renderer y) {
            error_renderers[x] = y;
            return *this;
        }
        ...
        spawner_config &spawner_config::set_impl(string_view name, config_value value) {
            ...
            if (opt == nullptr) {
                ...
                std::cerr << "*** failed to set config parameter " << name << ": " << to_string(err) << std::endl;
            } else {
                opt->store(value);
                auto category = opt->category();
                auto &dict = category == "global" ? content : content[category].as_dictionary();
        ...
        expected<settings> spawner_config::parse_config(std::istream &source) {
            config_option_set dummy;
            return parse_config(source, dummy);
        }

        expected<settings> spawner_config::parse_config(std::istream &source, const config_option_set &opts) {
            settings result;
        ...
        error spawner_config::parse_config(std::istream &source, const config_option_set &opts, settings &result) {
            if (!source)
        ...
        error spawner_config::extract_config_file_path(string_list &args) {
        ...
        error spawner_config::adjust_content() {
