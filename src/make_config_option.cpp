...
#define DEFAULT_META(type, parse_fun)                                                                                  \
    config_option::meta_state type##_meta_state {default_config_option_check<type>, default_config_option_store<type>, \
...
namespace nil {
    namespace actor {
        ...
        namespace {
            ...
            meta_state bool_neg_meta {detail::check_impl<bool>, bool_store_neg, bool_get_neg, nullptr,
            ...
            void store_timespan(void *ptr, const config_value &x) {
                *static_cast<size_t *>(ptr) = static_cast<size_t>(get<timespan>(x).count()) / Denominator;
            }
            template<uint64_t Denominator>
            ...
            meta_state us_res_meta {check_timespan, store_timespan<1000>, get_timespan<1000>, nullptr,
                                    select_config_value_access_t<timespan>::type_name()};

            meta_state ms_res_meta {check_timespan, store_timespan<1000000>, get_timespan<1000000>, nullptr,
                                    select_config_value_access_t<timespan>::type_name()};

        }    // namespace

        config_option make_negated_config_option(bool &storage, string_view category, string_view name,
