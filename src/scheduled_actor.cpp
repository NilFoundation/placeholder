    ...
    namespace actor {
        ...
        error scheduled_actor::default_exception_handler(pointer ptr, std::exception_ptr &x) {
            ...
            try {
                ...
                aout(ptr) << "*** unhandled exception: [id: " << ptr->id() << ", name: " << ptr->name()
                          << ", exception typeid: " << typeid(e).name() << "]: " << e.what() << std::endl;
            } catch (...) {
                aout(ptr) << "*** unhandled exception: [id: " << ptr->id() << ", name: " << ptr->name()
        ...
        stream_slot scheduled_actor::next_slot() {
            ...
            auto nslot = [](const stream_manager_map &x) -> stream_slot { return x.rbegin()->first + 1; };
            if (!stream_managers_.empty())
        ...
        invoke_message_result scheduled_actor::handle_open_stream_msg(mailbox_element &x) {
            ...
            auto fail = [&](sec y, const char *reason) {
                ...
                inbound_path::emit_irregular_shutdown(this, path_id, osm.prev_stage, make_error(y, reason));
                auto rp = make_response_promise();
        ...
        actor_clock::time_point scheduled_actor::advance_streams(actor_clock::time_point now) {
            ...
            auto bitmask = stream_ticks_.timeouts(now, {max_batch_delay_ticks_, credit_round_ticks_});
