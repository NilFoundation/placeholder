...
namespace {
    ...
    string_builder &operator<<(string_builder &builder, nil::actor::string_view str) {
...
namespace nil {
    namespace actor {
        ...
        std::string config_option_set::help_text(bool global_only) const {
            ...
            for (auto &category : categories) {
                ...
                for (auto i = rng.first; i != rng.second; ++i) {
                    ...
                    ACTOR_ASSERT(max_arg_size >= i->second.first.size());
        ...
        auto config_option_set::parse(settings &config, argument_iterator first, argument_iterator last) const
            ...
            for (auto i = first; i != last;) {
                ...
                if (i->compare(0, 2, "--") == 0) {
                    ...
                    auto name = assign_op == npos ? i->substr(2) : i->substr(2, assign_op - 2);
                    auto opt = cli_long_name_lookup(name);
                    if (opt == nullptr)
                        return {pec::not_an_option, i};
                    auto code =
                        consume(*opt,
                                assign_op == npos ? i->end() : i->begin() + static_cast<ptrdiff_t>(assign_op + 1),
                                i->end());
                    if (code != pec::success)
                        return {code, i};
                    ++i;
        ...
        config_option_set::parse_result config_option_set::parse(settings &config,
                                                                 const std::vector<string> &args) const {
            return parse(config, args.begin(), args.end());
        }

        config_option_set::option_pointer config_option_set::cli_long_name_lookup(string_view input) const {
            ...
            auto category_match = [&](const config_option &opt) {
                return sep == string::npos ? opt.has_flat_cli_name() : opt.category() == category;
            };
            return detail::ptr_find_if(
                opts_, [&](const config_option &opt) { return category_match(opt) && opt.long_name() == long_name; });
        }

        config_option_set::option_pointer config_option_set::cli_short_name_lookup(char short_name) const {
            return detail::ptr_find_if(
                opts_, [&](const config_option &opt) { return opt.short_names().find(short_name) != string::npos; });
        }

        config_option_set::option_pointer config_option_set::qualified_name_lookup(string_view category,
        ...
        config_option_set::option_pointer config_option_set::qualified_name_lookup(string_view name) const {
            auto sep = name.find('.');
            if (sep == string::npos)
                return nullptr;
