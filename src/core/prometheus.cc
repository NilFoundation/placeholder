...
namespace nil {
    namespace actor {
        ...
        namespace prometheus {
            ...
            static std::string to_str(nil::actor::metrics::impl::data_type dt) {
                switch (dt) {
                    case nil::actor::metrics::impl::data_type::GAUGE:
                        return "gauge";
                    case nil::actor::metrics::impl::data_type::COUNTER:
                        return "counter";
                    case nil::actor::metrics::impl::data_type::HISTOGRAM:
                        return "histogram";
                    case nil::actor::metrics::impl::data_type::DERIVE:
            ...
            static std::string to_str(const nil::actor::metrics::impl::metric_value &v) {
                switch (v.type()) {
                    case nil::actor::metrics::impl::data_type::GAUGE:
            ...
            static void add_name(std::ostream &s, const sstring &name, const std::map<sstring, sstring> &labels,
                                 const config &ctx) {
                s << name << "{";
             ...
             *   - if the current family (the metric family that we get from the shard and position) has the current
             * name:
            ...
            static future<> get_map_value(metrics_families_per_shard &vec) {
                ...
                return parallel_for_each(boost::irange(0u, smp::count), [&vec](auto cpu) {
                    return smp::submit_to(cpu, [] { return mi::get_values(); }).then([&vec, cpu](auto res) {
            ...
            class metric_family {
                ...
                metric_family(const sstring *name, uint32_t size, const mi::metric_family_info *family_info,
                              metric_family_iterator &state) :
                    _name(name),
                    _size(size), _family_info(family_info), _iterator_state(state) {
                }
            ...
            public:
                ...
                metric_family_iterator(const metric_family_iterator &o) :
                    _families(o._families), _positions(o._positions), _info(*this) {
                    next();
                }

                metric_family_iterator(metric_family_iterator &&o) :
                    _families(o._families), _positions(std::move(o._positions)), _info(*this) {
                    next();
                }

                metric_family_iterator(const metrics_families_per_shard &families, unsigned shards) :
                    _families(families), _positions(shards, 0), _info(*this) {
                    next();
                }

                metric_family_iterator(const metrics_families_per_shard &families, std::vector<size_t> &&positions) :
            ...
            void metric_family::foreach_metric(
                std::function<void(const mi::metric_value &, const mi::metric_info &)> &&f) {
            ...
            public:
                metric_family_range(const metrics_families_per_shard &families) :
                    _begin(families, smp::count), _end(metric_family_iterator(families, 0)) {
                }

                metric_family_range(const metric_family_iterator &b, const metric_family_iterator &e) :
                    _begin(b), _end(e) {
                }
            ...
            metric_family_iterator metrics_families_per_shard::find_bound(const sstring &family_name,
                                                                          comp_function comp) const {
                ...
                for (auto &shard_info : _data) {
                    ...
                    std::vector<mi::metric_family_metadata>::iterator it_b =
                        boost::range::upper_bound(metadata, family_name, comp);
            ...
            metric_family_iterator metrics_families_per_shard::upper_bound(const sstring &family_name) const {
                return find_bound(family_name,
                                  [](const sstring &a, const mi::metric_family_metadata &b) { return a < b.mf.name; });
            }
             ...
             */
            metric_family_range get_range(const metrics_families_per_shard &mf, const sstring &metric_family_name,
                                          bool prefix) {
            ...
            future<> write_text_representation(output_stream<char> &out, const config &ctx,
                                               const metric_family_range &m) {
                return nil::actor::async([&ctx, &out, &m]() mutable {
                    bool found = false;
                    for (metric_family &metric_family : m) {
                        auto name = ctx.prefix + "_" + metric_family.name();
                        found = false;
                        metric_family.foreach_metric(
                            [&out, &ctx, &found, &name, &metric_family](auto value, auto value_info) mutable {
                                std::stringstream s;
                                if (!found) {
            ...
            future<> write_protobuf_representation(output_stream<char> &out, const config &ctx,
                                                   metric_family_range &m) {
            ...
            public:
                ...
                future<std::unique_ptr<httpd::reply>> handle(const sstring &path, std::unique_ptr<httpd::request> req,
                    ...
                    rep->write_body((text) ? "txt" : "proto", [this, text, metric_family_name,
                                                               prefix](output_stream<char> &&s) {
                        return do_with(
                            metrics_families_per_shard(), output_stream<char>(std::move(s)),
                            [this, text, prefix, &metric_family_name](metrics_families_per_shard &families,
                                                                      output_stream<char> &s) mutable {
                                return get_map_value(families)
                                    .then([&s, &families, this, text, prefix, &metric_family_name]() mutable {
                                    ...
                                    .finally([&s]() mutable { return s.close(); });
                            });
                    });
            ...
            future<> add_prometheus_routes(distributed<http_server> &server, config ctx) {
                return server.invoke_on_all([ctx](http_server &s) { return add_prometheus_routes(s, ctx); });
            }
