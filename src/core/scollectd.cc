    ...
    namespace actor {
        ...
        void scollectd::type_instance_id::truncate(sstring &field, const char *field_desc) {
            if (field.size() > max_collectd_field_text_len) {
                ...
                sstring new_field(nil::actor::format(
                    "{}~{:d}", sstring(field.data(), max_collectd_field_text_len - suffix_len), _next_truncated_idx));

                logger.warn("Truncating \"{}\" to {} chars: \"{}\" -> \"{}\"", field_desc, max_collectd_field_text_len,
                            field, new_field);
                field = std::move(new_field);
            }
        }

        bool scollectd::type_instance_id::operator<(const scollectd::type_instance_id &id2) const {
            auto &id1 = *this;
            return std::tie(id1.plugin(), id1.plugin_instance(), id1.type(), id1.type_instance()) <
                   std::tie(id2.plugin(), id2.plugin_instance(), id2.type(), id2.type_instance());
        }
        bool scollectd::type_instance_id::operator==(const scollectd::type_instance_id &id2) const {
            auto &id1 = *this;
            return std::tie(id1.plugin(), id1.plugin_instance(), id1.type(), id1.type_instance()) ==
        ...
        namespace scollectd {
            ...
            ::nil::actor::logger logger("scollectd");
            ...
            registration::registration(const type_instance_id &id) :
                _id(id), _impl(nil::actor::metrics::impl::get_local_impl()) {
            }

            registration::registration(type_instance_id &&id) :
                _id(std::move(id)), _impl(nil::actor::metrics::impl::get_local_impl()) {
            }

            nil::actor::metrics::impl::metric_id to_metrics_id(const type_instance_id &id) {
                return nil::actor::metrics::impl::metric_id(
                    id.plugin(), id.type_instance(),
                    {{nil::actor::metrics::shard_label.name(), nil::actor::metrics::impl::shard()}});
            }
            ...
            enum class part_type : uint16_t {
                ...
                PluginInst = 0x0003,    // Plugin instance String  The plugin instance name to associate with subsequent
                                        // data values, e.g. "1"
                Type = 0x0004,        // Type String The type name to associate with subsequent data values, e.g. "cpu"
                TypeInst = 0x0005,    // Type instance     String  The type instance name to associate with subsequent
                                      // data values, e.g. "idle"
                Values = 0x0006,      // Values  other   Data values, see above
                Interval = 0x0007,    // Interval Numeric Interval used to set the "step" when creating new RRDs unless
                                      // rrdtool plugin forces StepSize. Also used to detect values that have timed out.
                IntervalHr =
                    0x0009,    // Interval (high resolution)  Numeric     The interval in which subsequent data
                               // values are collected. The interval is given in 2–30 seconds. New in Version 5.0.
            ...
            struct cpwriter {
                ...
                sstring get_type_instance(const nil::actor::metrics::impl::metric_id &id) {
                    if (id.labels().empty()) {
                        return id.name();
                    }
                    sstring res = id.name();
                    for (auto i : id.labels()) {
                        if (i.first != nil::actor::metrics::shard_label.name()) {
                            res += "-" + i.second;
                        }
                    }
                    return res;
                ...
                template<typename T>
                typename std::enable_if<std::is_integral<T>::value, cpwriter &>::type write(const T &t) {
                    T tmp = net::hton(t);
                ...
                void write_value(const nil::actor::metrics::impl::metric_value &v) {
                    switch (v.type()) {
                ...
                template<typename T>
                typename std::enable_if<std::is_integral<T>::value, cpwriter &>::type put(part_type type, T t) {
                    write(uint16_t(type));
                ...
                cpwriter &put(part_type type, const nil::actor::metrics::impl::metric_value &v) {
                ...
                cpwriter &put(const sstring &host, const nil::actor::metrics::impl::metric_id &id,
                    ...
                    put_cached(part_type::PluginInst,
                               id.instance_id() == per_cpu_plugin_instance ? to_sstring(this_shard_id()) :
                                                                             id.instance_id());
                ...
                cpwriter &put(const sstring &host, const duration &period, const type_instance_id &id,
                              const value_list &v) {
                    const auto ps = std::chrono::duration_cast<collectd_hres_duration>(period).count();
                ...
                cpwriter &put(const sstring &host, const duration &period, const type_id &type,
                              const nil::actor::metrics::impl::metric_id &id,
                              const nil::actor::metrics::impl::metric_value &v) {
                    const auto ps = std::chrono::duration_cast<collectd_hres_duration>(period).count();
                    put(host, id, type);
                    put(part_type::Values, v);
                    if (ps != 0) {
            ...
            void impl::add_polled(const type_instance_id &id, const shared_ptr<value_list> &values, bool enable) {
                // do nothing
                // add_polled is now implemented on the metrics layer
            }

            void impl::remove_polled(const type_instance_id &id) {
                nil::actor::metrics::impl::unregister_metric(to_metrics_id(id));
            }

            // explicitly send a type_instance value list (outside polling)
            future<> impl::send_metric(const type_instance_id &id, const value_list &values) {
                if (values.empty()) {
            ...
            future<> impl::send_notification(const type_instance_id &id, const sstring &msg) {
                cpwriter out;
            ...
            void impl::start(const sstring &host, const ipv4_addr &addr, const duration period) {
                ...
                _metrics.add_group(
                    "scollectd",
                    {
                        ...
                        sm::make_gauge("total_values", sm::description("current number of values reported"),
                                       [this] { return values().size(); }),
                        // records          value:GAUGE:0:U
                        sm::make_gauge("records", sm::description("number of records reported"),
                                       [this] { return values().size(); }),
                    });

                // FIXME: future is discarded
                (void)send_notification(type_instance_id("scollectd", per_cpu_plugin_instance, "network"),
                                        "daemon started");
                arm();
            }
            ...
            void impl::run() {
                ...
                typedef nil::actor::metrics::impl::value_vector::iterator value_iterator;
                typedef nil::actor::metrics::impl::metric_metadata_vector::iterator metadata_iterator;
                ...
                foreign_ptr<shared_ptr<nil::actor::metrics::impl::values_copy>> vals =
                    nil::actor::metrics::impl::get_values();
                ...
                auto send_packet = [this, ctxt, &values, metadata]() mutable {
                    ...
                    while (!out_of_space && mf < values.size()) {
                        while (i != values[mf].end()) {
                            if (i->type() == nil::actor::metrics::impl::data_type::HISTOGRAM) {
                                ++i;
                                ++md_iterator;
                                continue;
                            }
                ...
                (void)do_until(stop_when, send_packet).finally([this, vals = std::move(vals)]() mutable { arm(); });
            }
            ...
            void add_polled(const type_instance_id &id, const shared_ptr<value_list> &values, bool enabled) {
            ...
            void configure(const boost::program_options::variables_map &opts) {
                ...
                auto host = (opts["collectd-hostname"].as<std::string>() == "") ?
                                nil::actor::metrics::impl::get_local_impl()->get_config().hostname :
                                sstring(opts["collectd-hostname"].as<std::string>());

                // Now create send loops on each cpu
                for (unsigned c = 0; c < smp::count; c++) {
                    // FIXME: future is discarded
                    (void)smp::submit_to(c, [=]() { get_impl().start(host, addr, period); });
                }
            }
            ...
            static nil::actor::metrics::impl::register_ref get_register(const scollectd::type_instance_id &i) {
                nil::actor::metrics::impl::metric_id id = to_metrics_id(i);
                return nil::actor::metrics::impl::get_value_map().at(id.full_name()).at(id.labels());
            }

            std::vector<collectd_value> get_collectd_value(const scollectd::type_instance_id &id) {
                std::vector<collectd_value> vals;
                const nil::actor::metrics::impl::registered_metric &val = *get_register(id);
                vals.push_back(val());
                return vals;
            }
            ...
            std::vector<type_instance_id> plugin_instance_metrics::bound_ids() const {
                ...
                std::transform(_registrations.begin(), _registrations.end(), std::back_inserter(res),
                               [](const registration &r) { return r._id; });
                return res;
            }
