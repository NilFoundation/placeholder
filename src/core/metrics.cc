...
namespace nil {
    namespace actor {
        namespace metrics {
            ...
            metric_groups::metric_groups(std::initializer_list<metric_group_definition> mg) :
                _impl(impl::create_metric_groups()) {
                for (auto &&i : mg) {
                    add_group(i.name, i.metrics);
                }
            }
            metric_groups &metric_groups::add_group(const group_name_type &name,
            ...
            metric_group_definition::metric_group_definition(const group_name_type &name,
                                                             std::initializer_list<metric_definition>
                                                                 l) :
                name(name),
                metrics(l) {
            }
            ...
            bool label_instance::operator<(const label_instance &id2) const {
                auto &id1 = *this;
                return std::tie(id1.key(), id1.value()) < std::tie(id2.key(), id2.value());
            }
            ...
            boost::program_options::options_description get_options_description() {
                ...
                opts.add_options()("metrics-hostname",
            ...
            future<> configure(const boost::program_options::variables_map &opts) {
                ...
                return smp::invoke_on_all([c] { impl::get_local_impl()->set_config(c); });
            }
            ...
            namespace impl {
                ...
                metric_definition_impl::metric_definition_impl(metric_name_type name,
                                                               metric_type type,
                                                               metric_function f,
                                                               description d,
                                                               std::vector<label_instance>
                                                                   _labels) :
                    name(name),
                    type(type), f(f), d(d), enabled(true) {
                    for (auto i : _labels) {
                ...
                metric_groups_impl &metric_groups_impl::add_group(group_name_type name,
                                                                  const std::vector<metric_definition> &l) {
                ...
                bool metric_id::operator<(const metric_id &id2) const {
                ...
                shared_ptr<impl> get_local_impl() {
                    static thread_local auto the_impl = ::nil::actor::make_shared<impl>();
                    return the_impl;
                }
                ...
                foreign_ptr<values_reference> get_values() {
                    shared_ptr<values_copy> res_ref = ::nil::actor::make_shared<values_copy>();
                ...
                void impl::update_metrics_if_needed() {
                    if (_dirty) {
                        ...
                        _metadata = ::nil::actor::make_shared<metric_metadata>();

                        auto mt_ref = ::nil::actor::make_shared<metric_metadata>();
                        auto &mt = *(mt_ref.get());
                ...
                void impl::add_registration(const metric_id &id,
                                            const metric_type &type,
                                            metric_function f,
                                            const description &d,
                                            bool enabled) {
                    auto rm = ::nil::actor::make_shared<registered_metric>(id, f, enabled);
                    ...
                    if (_value_map.find(name) != _value_map.end()) {
                        ...
                        if (metric.info().type != type.base_type) {
                            throw std::runtime_error("registering metrics " + name +
