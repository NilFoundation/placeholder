...
namespace nil {
    namespace actor {
        ...
        static_assert(std::is_nothrow_constructible<data_source>::value);
        static_assert(std::is_nothrow_move_constructible<data_source>::value);

        static_assert(std::is_nothrow_constructible<data_sink>::value);
        static_assert(std::is_nothrow_move_constructible<data_sink>::value);

        static_assert(std::is_nothrow_constructible<temporary_buffer<char>>::value);
        static_assert(std::is_nothrow_move_constructible<temporary_buffer<char>>::value);

        static_assert(std::is_nothrow_constructible<input_stream<char>>::value);
        static_assert(std::is_nothrow_move_constructible<input_stream<char>>::value);

        static_assert(std::is_nothrow_constructible<output_stream<char>>::value);
        static_assert(std::is_nothrow_move_constructible<output_stream<char>>::value);
        ...
        private:
            ...
            void set_new_buffer_size(after_skip skip) {
                ...
                auto new_target = (unused_ratio_target::num * total - unused_ratio_target::den * unused) /
                                  (unused_ratio_target::den - unused_ratio_target::num);
        ...
        public:
            file_data_source_impl(file f, uint64_t offset, uint64_t len, file_input_stream_options options) :
                ...
                _current_read_ahead(get_initial_read_ahead()), _current_buffer_size(_options.buffer_size) {
                // prevent wraparounds
        ...
        public:
            file_data_source(file f, uint64_t offset, uint64_t len, file_input_stream_options options) :
                data_source(std::make_unique<file_data_source_impl>(std::move(f), offset, len, options)) {
            }
        };

        input_stream<char> make_file_input_stream(file f, uint64_t offset, uint64_t len,
                                                  file_input_stream_options options) {
            return input_stream<char>(file_data_source(std::move(f), offset, len, std::move(options)));
        }

        input_stream<char> make_file_input_stream(file f, uint64_t offset, file_input_stream_options options) {
            return make_file_input_stream(std::move(f), offset, std::numeric_limits<uint64_t>::max(),
                                          std::move(options));
        }

        input_stream<char> make_file_input_stream(file f, file_input_stream_options options) {
        ...
        public:
            file_data_sink_impl(file f, file_output_stream_options options) : _file(std::move(f)), _options(options) {
            ...
            virtual future<> put(temporary_buffer<char> buf) override {
                ...
                return _write_behind_sem.wait().then([this, pos, buf = std::move(buf)]() mutable {
                    ...
                    auto this_write_done = do_put(pos, std::move(buf)).finally([this] { _write_behind_sem.signal(); });
        ...
        public:
            future<> do_put(uint64_t pos, temporary_buffer<char> buf) noexcept {
                try {
                    ...
                    if ((buf.size() & (_file.disk_write_dma_alignment() - 1)) != 0) {
                        ...
                        auto tmp = allocate_buffer(
                            align_up(buf.size(), static_cast<size_t>(_file.disk_write_dma_alignment())));
                    ...
                    return _file.dma_write(pos, p, buf_size, _options.io_priority_class)
                        .then([this, pos, buf = std::move(buf), truncate, buf_size](size_t size) mutable {
            ...
            future<> wait() noexcept {
                ...
                return _write_behind_sem.wait(_options.write_behind)
                    .then([this] { return std::exchange(_background_writes_done, make_ready_future<>()); })
                    .finally([this] { _write_behind_sem.signal(_options.write_behind); });
            }

        public:
            virtual future<> flush() override {
                return wait().then([this] { return _file.flush(); });
            }
            virtual future<> close() noexcept override {
                return wait().finally([this] { return _file.close(); });
            }
        };
