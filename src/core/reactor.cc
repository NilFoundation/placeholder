...
namespace nil { namespace actor {
    ...
    future<> reactor::rename_priority_class(io_priority_class pc, sstring new_name) noexcept {
        ...
        return futurize_invoke([pc, new_name = std::move(new_name)]() mutable {
            ...
            try {
                ...
                sched_logger.error("exception while trying to rename priority group with id {} to \"{}\" ({})", pc.id(),
                                   new_name, std::current_exception());
    ...
    future<temporary_buffer<char>> reactor::do_read_some(pollable_fd_state &fd, internal::buffer_allocator *ba) {
    ...
    void reactor::signals::handle_signal(int signo, noncopyable_function<void()> &&handler) {
        ...
        struct sigaction sa;
        sa.sa_sigaction = [](int sig, siginfo_t *info, void *p) { engine()._backend->signal_received(sig, info, p); };
    ...
    reactor::task_queue::task_queue(unsigned id, sstring name, float shares) :
        _shares(std::max(shares, 1.0f)), _reciprocal_shares_times_2_power_32((uint64_t(1) << 32) / _shares), _id(id),
        _ts(std::chrono::steady_clock::now()), _name(name) {
        register_stats();
    }
        ...
        new_metrics.add_group(
            "scheduler",
            {
                sm::make_counter(
                    ...
                    sm::description("Accumulated runtime of this task queue; an increment rate of 1000ms per second "
                                    "indicates full utilization"),
                    {group_label}),
                sm::make_counter(
                    "waittime_ms",
                    [this] { return std::chrono::duration_cast<std::chrono::milliseconds>(_waittime).count(); },
                    sm::description("Accumulated waittime of this task queue; an increment rate of 1000ms per second "
                ...
                sm::make_counter(
                    ...
                    sm::description("Accumulated starvation time of this task queue; an increment rate of 1000ms per "
                ...
                sm::make_gauge("shares", [this] { return _shares; }, sm::description("Shares allocated to this queue"),
                               {group_label}),
                sm::make_derive("time_spent_on_task_quota_violations_ms",
    ...
    future<file> reactor::open_file_dma(std::string_view nameref, open_flags flags,
        ...
        return do_with(
            static_cast<int>(flags), std::move(options), [this, nameref](auto &open_flags, file_open_options &options) {
    ...
    future<file> reactor::open_directory(std::string_view name) noexcept {
        ...
        return futurize_invoke([name, this] {
            ...
            return _thread_pool
                ...
                .then([](shared_ptr<file_impl> file_impl) { return make_ready_future<file>(std::move(file_impl)); });
        });
    }
        ...
        _metric_groups.add_group(
            "reactor",
            {
                ...
                sm::make_derive(
                    ...
                    sm::description("Total steal time, the time in which some other process was running while =nil; Actor "
        ...
        _metric_groups.add_group(
            "reactor",
            {
                ...
                make_derive(
                    ...
                    description("Counts bytes read from disk file streams.  A high rate indicates high disk activity."
    ...
    void smp_message_queue::submit_item(shard_id t, smp_timeout_clock::time_point timeout,
        ...
        (void)get_units(sem, 1, timeout)
            .then_wrapped([this, item = std::move(item)](future<smp_service_group_semaphore_units> units_fut) mutable {
                if (units_fut.failed()) {
...
namespace nil { namespace actor {
    ...
    boost::program_options::options_description reactor::get_options_description(reactor_config cfg) {
        ...
        opts.add_options()("network-stack", bpo::value<std::string>(),
            ...
            "overprovisioned",
            "run in an overprovisioned environment (such as docker or a laptop); equivalent to --idle-poll-time-us 0 "
            ...
            " Note that if the seastar_memory logger is set to debug or trace level, the diagnostics will be logged "
    ...
    boost::program_options::options_description smp::get_options_description() {
        ...
        opts.add_options()("smp,c", bpo::value<unsigned>(), "number of threads (default: one per CPU)")(
            ...
            ("num-io-queues", bpo::value<unsigned>(),
             "Number of IO queues. Each IO unit will be responsible for a fraction of the IO requests. Defaults to the "
             "number of threads")("num-io-groups", bpo::value<unsigned>(),
                ...
                "Maximum amount of concurrent requests to be sent to the disk. Defaults to 128 times the number of IO "
                "queues")
#else
            ("max-io-requests", bpo::value<unsigned>(),
        ...
        struct io_group::config generate_group_config(dev_t devid, unsigned nr_groups) const noexcept {
            ...
            if (!_capacity) {
                ...
                if (max_iops != std::numeric_limits<uint64_t>::max()) {
                    cfg.max_req_count =
                        io_queue::read_request_base_count * per_io_group(max_iops * latency_goal().count(), nr_groups);
                }
            } else {
    ...
    void smp::configure(boost::program_options::variables_map configuration, reactor_config reactor_cfg) {
        ...
        sigset_t sigs;
        sigfillset(&sigs);
        for (auto sig : {SIGHUP, SIGQUIT, SIGILL, SIGABRT, SIGFPE, SIGSEGV, SIGALRM, SIGCONT, SIGSTOP, SIGTSTP, SIGTTIN,
                         SIGTTOU}) {
            sigdelset(&sigs, sig);
        }
        ...
        for (i = 1; i < smp::count; i++) {
            ...
            create_thread([configuration, &disk_config, hugepages_path, i, allocation, assign_io_queue, alloc_io_queue,
                           thread_affinity, heapprof_enabled, mbind, backend_selector, reactor_cfg] {
                try {
                    ...
                    .then_wrapped([path = w.path, cleanup = std::move(w.cleanup)](future<file> f) {
                        try {
                            ...
                            if (e.code() == std::error_code(EINVAL, std::system_category())) {
                                report_exception(
                                    format("Could not open file at {}. Does your filesystem support O_DIRECT?", path),
    ...
    void reactor::allocate_scheduling_group_specific_data(scheduling_group sg, scheduling_group_key key) {
        ...
        this_sg.specific_vals[key.id()] = aligned_alloc(sg_data.scheduling_group_key_configs[key.id()].alignment,
        ...
        return parallel_for_each(_task_queues, [this, cfg, key](std::unique_ptr<task_queue> &tq) {
            if (tq) {
                ...
                return with_scheduling_group(sg,
                                             [this, key, sg]() { allocate_scheduling_group_specific_data(sg, key); });
            }
            return make_ready_future();
        });
    }
        ...
        return with_scheduling_group(sg,
                                     [this, sg]() {
                                         ...
                                         for (unsigned long key_id = 0;
                                              key_id < sg_data.scheduling_group_key_configs.size(); key_id++) {
    ...
    void internal::no_such_scheduling_group(scheduling_group sg) {
        throw std::invalid_argument(
            format("The scheduling group does not exist ({})", internal::scheduling_group_index(sg)));
    }
    ...
    future<scheduling_group_key> scheduling_group_key_create(scheduling_group_key_config cfg) noexcept {
        ...
        return smp::invoke_on_all([key, cfg] { return engine().init_new_scheduling_group_key(key, cfg); }).then([key] {
    ...
    future<> destroy_scheduling_group(scheduling_group sg) noexcept {
        if (sg == default_scheduling_group()) {
            return make_exception_future<>(
                make_backtraced_exception_ptr<std::runtime_error>("Attempt to destroy the default scheduling group"));
        }
        if (sg == current_scheduling_group()) {
            return make_exception_future<>(
                make_backtraced_exception_ptr<std::runtime_error>("Attempt to destroy the current scheduling group"));
        }
    ...
    future<> rename_scheduling_group(scheduling_group sg, sstring new_name) noexcept {
        if (sg == default_scheduling_group()) {
            return make_exception_future<>(
                make_backtraced_exception_ptr<std::runtime_error>("Attempt to rename the default scheduling group"));
        }
    ...
    namespace internal {
        ...
        std::ostream &operator<<(std::ostream &os, const stall_report &sr) {
            ...
            return os << format("{} stalls, {} ms stall time, {} ms run time", sr.kernel_stalls, to_ms(sr.stall_time),
                                to_ms(sr.run_wall_time));
        }
