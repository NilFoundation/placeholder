...
namespace nil::actor::detail {
    ...
    impl::impl(scheduled_actor *self) : super(self) {
        ...
        buffer_capacity_ =
            get_or(cfg, "stream.size-policy.buffer-capacity", defaults::stream::size_policy::buffer_capacity);
        bytes_per_batch_ =
            get_or(cfg, "stream.size-policy.bytes-per-batch", defaults::stream::size_policy::bytes_per_batch);
    }
    ...
    credit_controller::assignment impl::compute_bridge() {
        ACTOR_ASSERT(batch_size_ > 0);
        ACTOR_ASSERT(buffer_size_ > batch_size_);
        return {buffer_size_, batch_size_};
    }
