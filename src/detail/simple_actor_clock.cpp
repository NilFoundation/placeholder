...
namespace nil::actor::detail {
    ...
    void simple_actor_clock::set_ordinary_timeout(time_point t, abstract_actor *self, std::string type, uint64_t id) {
        new_schedule_entry<ordinary_timeout>(t, self->ctrl(), type, id);
    }

    void simple_actor_clock::set_multi_timeout(time_point t, abstract_actor *self, std::string type, uint64_t id) {
        new_schedule_entry<multi_timeout>(t, self->ctrl(), type, id);
    }

    void simple_actor_clock::set_request_timeout(time_point t, abstract_actor *self, message_id id) {
        new_schedule_entry<request_timeout>(t, self->ctrl(), id);
    }

    void simple_actor_clock::cancel_ordinary_timeout(abstract_actor *self, std::string type) {
        ordinary_timeout_cancellation tmp {self->id(), std::move(type)};
        handle(tmp);
    }

    void simple_actor_clock::cancel_request_timeout(abstract_actor *self, message_id id) {
    ...
    void simple_actor_clock::schedule_message(time_point t, strong_actor_ptr receiver, mailbox_element_ptr content) {
        new_schedule_entry<actor_msg>(t, std::move(receiver), std::move(content));
    }

    void simple_actor_clock::schedule_message(time_point t, group target, strong_actor_ptr sender, message content) {
        new_schedule_entry<group_msg>(t, std::move(target), std::move(sender), std::move(content));
    }
    ...
    void simple_actor_clock::ship(delayed_event &x) {
        switch (x.subtype) {
            case ordinary_timeout_type: {
                ...
                auto &self = dref.self;
                self->get()->eq_impl(make_message_id(), self, nullptr, timeout_msg {dref.type, dref.id});
                break;
            }
            ...
            case group_msg_type: {
                ...
                if (dst)
                    dst->enqueue(std::move(dref.sender), make_message_id(), std::move(dref.content), nullptr);
                break;
            }
            default:
                break;
    ...
    void simple_actor_clock::handle(const ordinary_timeout_cancellation &x) {
        auto pred = [&](const actor_lookup_map::value_type &kvp) {
            ...
            return y.subtype == ordinary_timeout_type && x.type == static_cast<const ordinary_timeout &>(y).type;
        };
        cancel(x.aid, pred);
    }
    ...
    void simple_actor_clock::handle(const request_timeout_cancellation &x) {
        auto pred = [&](const actor_lookup_map::value_type &kvp) {
            auto &y = *kvp.second->second;
            return y.subtype == request_timeout_type && x.id == static_cast<const request_timeout &>(y).id;
        };
        cancel(x.aid, pred);
    }
    ...
    void simple_actor_clock::add_schedule_entry(time_point t, std::unique_ptr<ordinary_timeout> x) {
        auto aid = x->self->id();
        auto type = x->type;
        auto pred = [&](const actor_lookup_map::value_type &kvp) {
            auto &y = *kvp.second->second;
            return y.subtype == ordinary_timeout_type && static_cast<const ordinary_timeout &>(y).type == type;
        };
