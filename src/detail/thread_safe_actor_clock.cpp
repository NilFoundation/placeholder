...
namespace nil {
    namespace actor {
        namespace detail {
            ...
            void thread_safe_actor_clock::set_multi_timeout(time_point t, abstract_actor *self, atom_value type,
                                                            uint64_t id) {
                push(new multi_timeout(t, self->ctrl(), type, id));
            }

            void thread_safe_actor_clock::cancel_ordinary_timeout(abstract_actor *self, atom_value type) {
            ...
            void thread_safe_actor_clock::schedule_message(time_point t, group target, strong_actor_ptr sender,
                                                           message content) {
                push(new group_msg(t, std::move(target), std::move(sender), std::move(content)));
            }
