...
namespace nil::actor::detail {
    ...
    abstract_ini_consumer::abstract_ini_consumer(abstract_ini_consumer *parent) : parent_(parent) {
        // nop
    }
    ...
    ini_map_consumer::ini_map_consumer(abstract_ini_consumer *parent) : super(parent), i_(xs_.end()) {
        ACTOR_ASSERT(parent != nullptr);
    }

    ini_map_consumer::ini_map_consumer(ini_map_consumer &&other) : super(other.parent()) {
    }
    ...
    void ini_map_consumer::value_impl(config_value &&x) {
        ACTOR_ASSERT(i_ != xs_.end());
        i_->second = std::move(x);
    }

    // -- ini_list_consumer --------------------------------------------------------

    ini_list_consumer::ini_list_consumer(abstract_ini_consumer *parent) : super(parent) {
        ACTOR_ASSERT(parent != nullptr);
    }

    ini_list_consumer::ini_list_consumer(ini_list_consumer &&other) : super(other.parent()), xs_(std::move(other.xs_)) {
        // nop
    }
    ...
    ini_value_consumer::ini_value_consumer(abstract_ini_consumer *parent) : super(parent) {
        // nop
    }
    ...
    ini_category_consumer::ini_category_consumer(ini_consumer *parent, std::string category) :
        super(parent), category_(std::move(category)) {
        ACTOR_ASSERT(parent_ != nullptr);
    }

    ini_category_consumer::ini_category_consumer(ini_category_consumer &&other) :
        super(other.parent()), category_(std::move(other.category_)), xs_(std::move(other.xs_)),
    ...
    void ini_consumer::value_impl(config_value &&x) {
        ...
        if (dict == nullptr) {
            warnings_.emplace_back(make_error(pec::type_mismatch, "expected a dictionary at top level"));
            return;
        }
