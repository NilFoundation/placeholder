...
namespace nil { namespace actor {
    ...
    namespace internal {
        ...
        log_buf::log_buf() : _begin(new char[512]), _end(_begin + 512), _current(_begin), _own_buf(true) {
        }

        log_buf::log_buf(char *external_buf, size_t size) noexcept :
            _begin(external_buf), _end(_begin + size), _current(_begin), _own_buf(false) {
        }
    ...
    void validate(boost::any &v, const std::vector<std::string> &values, logger_timestamp_style *target_type, int) {
    ...
    void validate(boost::any &v, const std::vector<std::string> &values, logger_ostream_type *target_type, int) {
        using namespace boost::program_options;
        validators::check_first_occurrence(v);
        auto s = validators::get_single_string(values);
        if (s == "none") {
    ...
    logger::rate_limit::rate_limit(std::chrono::milliseconds interval) : _interval(interval), _next(clock::now()) {
    }
        ...
        if (is_syslog_enabled) {
            ...
            *it = '\0';
            static array_map<int, 20> level_map = {
                {int(log_level::debug), LOG_DEBUG},  {int(log_level::info), LOG_INFO},
                {int(log_level::trace), LOG_DEBUG},    // no LOG_TRACE
                {int(log_level::warn), LOG_WARNING}, {int(log_level::error), LOG_ERR},
            };
    ...
    void logger::set_ostream(std::ostream &out) noexcept {
        _out = &out;
    }

    void logger::set_ostream_enabled(bool enabled) noexcept {
        _ostream.store(enabled, std::memory_order_relaxed);
    }

    void logger::set_stdout_enabled(bool enabled) noexcept {
        _ostream.store(enabled, std::memory_order_relaxed);
    }

    void logger::set_syslog_enabled(bool enabled) noexcept {
    ...
    void logger_registry::set_all_loggers_level(log_level level) {
    ...
    std::vector<sstring> logger_registry::get_all_logger_names() {
        std::lock_guard<std::mutex> g(_mutex);
        ...
        for (const auto &pair : s.logger_levels) {
            try {
                ...
                throw std::runtime_error(
                    nil::actor::format("Unknown logger '{}'. Use --help-loggers to list available loggers.", pair.first));
            }
        }
    ...
    namespace log_cli {
        ...
        bpo::options_description get_options_description() {
            ...
            opts.add_options()("default-log-level",
                ...
                "logger-stdout-timestamps",
                bpo::value<logger_timestamp_style>()->default_value(logger_timestamp_style::real),
                "Select timestamp style for stdout logs: none|boot|real")(
                "log-to-stdout", bpo::value<bool>()->default_value(true),
                "Send log output to output stream, as selected by --logger-ostream-type")(
                "logger-ostream-type", bpo::value<logger_ostream_type>()->default_value(logger_ostream_type::stderr),
                "Send log output to: none|stdout|stderr")("log-to-syslog", bpo::value<bool>()->default_value(false),
...
namespace std {
    std::ostream &operator<<(std::ostream &out, const std::exception_ptr &eptr) {
        ...
        try {
            ...
            if (tp) {
                out << nil::actor::pretty_type_name(*tp);
            } else {
            ...
            try {
                throw;
            } catch (const nil::actor::nested_exception &ne) {
    ...
    std::ostream &operator<<(std::ostream &out, const std::exception &e) {
        return out << nil::actor::pretty_type_name(typeid(e)) << " (" << e.what() << ")";
    }

    std::ostream &operator<<(std::ostream &out, const std::system_error &e) {
        return out << nil::actor::pretty_type_name(typeid(e)) << " (error " << e.code() << ", " << e.what() << ")";
    }
