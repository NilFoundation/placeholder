              ...
              public:
                ...
                zkevm_end_transaction_bbf(
                    ...
                    if constexpr (stage == GenerationStage::CONSTRAINTS) {
                        ...
                        // // PC transition constrain(current_state.gas(0) -
                        // current_state.gas_next() - 1);                 // GAS
                        // transition constrain(current_state.stack_size(0) -
                        // current_state.stack_size_next());       // stack_size
                        // transition constrain(current_state.memory_size(0) -
                        // current_state.memory_size_next());     // memory_size
                        // transition constrain(current_state.rw_counter_next() -
                        // current_state.rw_counter(0));   // rw_counter transition
                        // constrain(current_state.opcode_next() - next_opcode); // Next
                        // opcode restrictions
                        constrain((current_state.opcode_next() -
                                   TYPE(std::size_t(opcode_to_number(
                                       zkevm_opcode::start_transaction)))) *
                                  (current_state.opcode_next() -
                                   TYPE(std::size_t(opcode_to_number(
                                       zkevm_opcode::end_block)))));  // Next opcode
                                                                      // restrictions

                        std::vector<TYPE> tmp;
                        tmp = log_table<FieldType, stage>::log_block_lookup(block_id,
                                                                            tx_id);
                        lookup(tmp, "zkevm_block_logs");
                    }
                }
            };
            ...
            class zkevm_end_transaction_operation : public opcode_abstract<FieldType> {
              public:
                virtual void fill_context(
                    ...
                    zkevm_end_transaction_bbf<FieldType, GenerationStage::ASSIGNMENT>
                ...
                virtual void fill_context(
                    ...
                    const opcode_input_type<FieldType, GenerationStage::CONSTRAINTS>
                        &current_state) override {
                    zkevm_end_transaction_bbf<FieldType, GenerationStage::CONSTRAINTS>
                        bbf_obj(context, current_state);
                }
