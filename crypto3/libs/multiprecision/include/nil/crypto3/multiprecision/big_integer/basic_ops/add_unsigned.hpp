...
namespace nil::crypto3::multiprecision::detail {
    ...
    inline constexpr void add_unsigned_constexpr(big_integer<Bits>& result,
        ...
        if (s == 1) {
            ...
            double_limb_type mask = big_integer<Bits>::upper_limb_mask;
            if (r & ~mask) {
                result = r & mask;
        ...
        typename big_integer<Bits>::const_limb_pointer pb = b.limbs();
    ...
    //
    template<unsigned Bits>
    inline constexpr void subtract_unsigned_constexpr(big_integer<Bits>& result,
                                                      const big_integer<Bits>& a,
                                                      const big_integer<Bits>& b) noexcept {
        BOOST_ASSERT(a >= b);
        ...
        // First where a and b overlap:
        for (std::size_t i = 0; i < s; ++i) {
            borrow = static_cast<double_limb_type>(pa[i]) - static_cast<double_limb_type>(pb[i]) -
                     borrow;
            pr[i] = static_cast<limb_type>(borrow);
            borrow = (borrow >> big_integer<Bits>::limb_bits) & 1u;
        }
    ...
    inline constexpr void subtract_unsigned(big_integer<Bits>& result, const big_integer<Bits>& a,
...
#endif
        {
            ...
            for (; i + 8 <= m; i += 8) {
                ...
                borrow =
                    _subborrow_u64(borrow, *reinterpret_cast<const unsigned long long*>(pa + i + 2),
                                   *reinterpret_cast<const unsigned long long*>(pb + i + 2),
                                   reinterpret_cast<unsigned long long*>(pr + i + 2));
                borrow =
                    _subborrow_u64(borrow, *reinterpret_cast<const unsigned long long*>(pa + i + 4),
                                   *reinterpret_cast<const unsigned long long*>(pb + i + 4),
                                   reinterpret_cast<unsigned long long*>(pr + i + 4));
                borrow =
                    _subborrow_u64(borrow, *reinterpret_cast<const unsigned long long*>(pa + i + 6),
