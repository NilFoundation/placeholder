...
contract modular_lookup_argument_circuit7_lpc is ILookupArgument{
//library modular_lookup_argument_circuit7_lpc{
    ...
    function verify(
        ...
        state.V_L_value = basic_marshalling.get_uint256_be(zvalues, 0x80 + 0x400);
        state.V_L_shifted_value = basic_marshalling.get_uint256_be(zvalues, 0x80 + 0x400 + 0x20);
        ...
        tr_state.current_challenge = tr_state_before;
        {
			...
			uint256 prod;
// -- lookup 0 is inlined -- 
		state.selector_value=basic_marshalling.get_uint256_be(blob, 480);
		...
		sum = 0;
		prod = basic_marshalling.get_uint256_be(blob, 768);
		sum = addmod(sum, prod, modulus);
		...
		state.theta_acc = mulmod(state.theta_acc, state.theta, modulus);
		sum = 0;
		prod = basic_marshalling.get_uint256_be(blob, 864);
		sum = addmod(sum, prod, modulus);
...
// -- /lookup 0 is inlined -- 
// -- lookup 1 is inlined -- 
		state.selector_value=basic_marshalling.get_uint256_be(blob, 512);
...
// -- /lookup 1 is inlined -- 
// -- lookup 2 is inlined -- 
		state.selector_value=basic_marshalling.get_uint256_be(blob, 544);
		l = mulmod( 3,state.selector_value, modulus);
		state.theta_acc=state.theta;
		sum = 0;
		prod = basic_marshalling.get_uint256_be(blob, 960);
		prod = mulmod(prod, basic_marshalling.get_uint256_be(blob, 992), modulus);
		sum = addmod(sum, prod, modulus);
...
// -- /lookup 2 is inlined -- 
			state.selector_value = basic_marshalling.get_uint256_be(blob, 640);
			state.shifted_selector_value = basic_marshalling.get_uint256_be(blob, 672);
			l = mulmod( 1, state.selector_value, modulus);
			...
			state.theta_acc = mulmod(state.theta_acc, state.theta, modulus);
			l = addmod( l, mulmod(state.selector_value,  mulmod( state.theta_acc, basic_marshalling.get_uint256_be(blob, 64), modulus), modulus), modulus);
			state.l_shifted = addmod( state.l_shifted, mulmod(state.shifted_selector_value, mulmod( state.theta_acc, basic_marshalling.get_uint256_be(blob, 96), modulus), modulus), modulus);
			state.theta_acc = mulmod(state.theta_acc, state.theta, modulus);
			l = addmod( l, mulmod(state.selector_value,  mulmod( state.theta_acc, basic_marshalling.get_uint256_be(blob, 128), modulus), modulus), modulus);
			state.l_shifted = addmod( state.l_shifted, mulmod(state.shifted_selector_value, mulmod( state.theta_acc, basic_marshalling.get_uint256_be(blob, 160), modulus), modulus), modulus);
			state.theta_acc = mulmod(state.theta_acc, state.theta, modulus);
			...
			state.theta_acc = mulmod(state.theta_acc, state.theta, modulus);
			l = addmod( l, mulmod(state.selector_value,  mulmod( state.theta_acc, basic_marshalling.get_uint256_be(blob, 256), modulus), modulus), modulus);
			state.l_shifted = addmod( state.l_shifted, mulmod(state.shifted_selector_value, mulmod( state.theta_acc, basic_marshalling.get_uint256_be(blob, 288), modulus), modulus), modulus);
			state.theta_acc = mulmod(state.theta_acc, state.theta, modulus);
			l = addmod( l, mulmod(state.selector_value,  mulmod( state.theta_acc, basic_marshalling.get_uint256_be(blob, 320), modulus), modulus), modulus);
			state.l_shifted = addmod( state.l_shifted, mulmod(state.shifted_selector_value, mulmod( state.theta_acc, basic_marshalling.get_uint256_be(blob, 352), modulus), modulus), modulus);
			state.theta_acc = mulmod(state.theta_acc, state.theta, modulus);
			...
			state.selector_value = basic_marshalling.get_uint256_be(blob, 576);
			state.shifted_selector_value = basic_marshalling.get_uint256_be(blob, 608);
			l = mulmod( 2, state.selector_value, modulus);
