...
library modular_commitment_scheme_circuit7_lpc {
    ...
    uint256 constant _eta = 4084459375749107065634815209207353736755634954064356355261228999139324100117;
    uint64  constant fixed_batch_size = 15;
    bytes constant point_ids = hex"00010001000100010001000100010001000100000000000100010203040500010607050000010000000000000000000000000000000000000000000000000000000108000108000108000108000108000108000108000108000108"; // 1 byte -- point id
    bytes constant poly_points_num = hex"003500160001000100010002000100010009"; // 2 byte lengths
    bytes constant poly_ids = hex""; // 2 byte poly_id 2 byte
    ...
    function prepare_eval_points(uint256[unique_points] memory result, uint256 xi) internal view {
		...
		result[2] = mulmod(xi, field.pow_small(inversed_omega, 7, modulus), modulus);
		...
		result[5] = mulmod(xi, inversed_omega, modulus);
		result[6] = mulmod(xi, field.pow_small(omega, 2, modulus), modulus);
		result[7] = mulmod(xi, field.pow_small(omega, 3, modulus), modulus);
		result[8] = mulmod(xi, field.pow_small(omega, 14, modulus), modulus);
    ...
    function prepare_eta_U(uint256 theta) internal pure returns (uint256 result){
		result = addmod(0x6ab2b6829e0fa3eb87bf05b1c8bc64c42ba59de957638b3188bfa874640d392, mulmod(result, theta, modulus), modulus);
		result = addmod(0x352f37e4086ac104514bd1637057cca0143cf7882e04e1af59a18ae1f3966023, mulmod(result, theta, modulus), modulus);
		result = addmod(0x3bda634c324bbb4309c7c1be8ce392ec56f75166c37b1a62722d856939d733b5, mulmod(result, theta, modulus), modulus);
		result = addmod(0x34ad730e63fc6c8bf122257937c39fa887b130f89230fd21ea9b3984741996e3, mulmod(result, theta, modulus), modulus);
		result = addmod(0xca2dbc1e851dcfbb599114365231c978016646d5d081ca2a3521c620240b566, mulmod(result, theta, modulus), modulus);
		result = addmod(0x2447612ad4640b089196a1f7f8b3647eb0cd0c44b511c01d359c649988d52aec, mulmod(result, theta, modulus), modulus);
		result = addmod(0x33af363829205feda2d2d72f2012b7fa56c91f77ae02f497e63bf8b029b494c8, mulmod(result, theta, modulus), modulus);
		result = addmod(0x18ceaa7f8c94dac87d6ee85038548403972d356b222ca8791b17d9aee6e43c26, mulmod(result, theta, modulus), modulus);
		result = addmod(0x11d79aef3b0cd250d59e28494d993306695a4670043f16a503a4e50167768678, mulmod(result, theta, modulus), modulus);
		result = addmod(0x9b61c754310593fc186f0b52922da2ef5ff2eb38b8d1facbbad35d7307cf6c2, mulmod(result, theta, modulus), modulus);
		result = addmod(0x2c545f75ecb6192fa4843ed8e66a0d44dd44cc0d1a878b136ba29f8229bad95f, mulmod(result, theta, modulus), modulus);
		result = addmod(0x791f6244acf072873c9755e5c24d97862fc620a96f6f0cae2e1f97a0ff6ace4, mulmod(result, theta, modulus), modulus);
		result = addmod(0x1b0b17e8c087f4aa92856cacd4373de312c30e3fd77fcb2c9ce61b3d8afbb979, mulmod(result, theta, modulus), modulus);
		result = addmod(0x15e6afc5606174dcabd08d6a96e2f6f30f4d691f80394fcc1e9ef2c110b6f779, mulmod(result, theta, modulus), modulus);
		result = addmod(0x356bdd2c3ba21e97630d4d1c31596964ad8e97e0002fa549292035947b2f71a7, mulmod(result, theta, modulus), modulus);
