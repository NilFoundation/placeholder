...
contract modular_gate_argument_circuit4_lpc is IGateArgument{
...
// constraint 0
		...
		prod = mulmod(prod, basic_marshalling.get_uint256_be(blob, 352), modulus);
		sum = addmod(sum, prod, modulus);
		sum = mulmod(sum, theta_acc, modulus);
		theta_acc = mulmod(theta, theta_acc, modulus);
		gate = addmod(gate, sum, modulus);
		gate = mulmod(gate, basic_marshalling.get_uint256_be(blob, 224), modulus);
