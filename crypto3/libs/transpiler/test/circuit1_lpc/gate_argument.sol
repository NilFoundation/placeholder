...
contract modular_gate_argument_circuit1_lpc is IGateArgument{
...
// constraint 0
		...
		prod = basic_marshalling.get_uint256_be(blob, 96);
		sum = addmod(sum, prod, modulus);
...
// constraint 0
		...
		theta_acc = mulmod(theta, theta_acc, modulus);
		gate = addmod(gate, sum, modulus);
		gate = mulmod(gate, basic_marshalling.get_uint256_be(blob, 32), modulus);
		F = addmod(F, gate, modulus);
