...
library modular_commitment_scheme_circuit1_lpc {
    ...
    uint256 constant _eta = 16659299798699156861902323958777682211991300444906294469152963499217430683477;
    uint64  constant fixed_batch_size = 4;
    bytes constant point_ids = hex"00010001000000000000000000"; // 1 byte -- point id
    bytes constant poly_points_num = hex"000b0002"; // 2 byte lengths
    bytes constant poly_ids = hex"00000040008000c001000140018001c002000240028000000040"; // 2 byte poly_id 2 byte
    ...
    function prepare_eta_U(uint256 theta) internal pure returns (uint256 result){
		result = addmod(0x0, mulmod(result, theta, modulus), modulus);
		result = addmod(0x143a9771440ec6a7b7feb0e2fadedb95b13760948d4ca2d6cb0dccd0d547c75d, mulmod(result, theta, modulus), modulus);
		result = addmod(0x187deb12aad9ed7728f0b014a12fb5c3f23ed5171da00ed8217f8e44726303e9, mulmod(result, theta, modulus), modulus);
		result = addmod(0x19442a77741df7c6d0961c506ecb37282b7c154a5e4630cded7d6f467cb59a31, mulmod(result, theta, modulus), modulus);
