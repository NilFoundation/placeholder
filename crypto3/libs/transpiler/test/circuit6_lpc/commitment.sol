...
library modular_commitment_scheme_circuit6_lpc {
    ...
    uint256 constant _eta = 23505150783261186445366348225085503873410955659436558695054991949443893866022;
    uint64  constant fixed_batch_size = 8;
    bytes constant point_ids = hex"000100010001000100010001000002000000010000000000000000000000000000000000000103000103000103000103000103000103000103"; // 1 byte -- point id
    bytes constant poly_points_num = hex"0023000e00010007"; // 2 byte lengths
    bytes constant poly_ids = hex"00000040008000c001000140018001c002000240028002c003000340038003c004000440048004c005000540058005c006000640068006c007000740078007c008000840088000000040008000c001000140028007000740078007c0080008400880020007000740078007c0080008400880"; // 2 byte poly_id 2 byte
    ...
    function prepare_eval_points(uint256[unique_points] memory result, uint256 xi) internal view {
		...
		result[3] = mulmod(xi, field.pow_small(inversed_omega, 2, modulus), modulus);
    ...
    function prepare_eta_U(uint256 theta) internal pure returns (uint256 result){
		result = addmod(0xc4338791c388ea87719dd202afd38799d1dc750b4cf9ccd62c4d243f0ad7dd9, mulmod(result, theta, modulus), modulus);
		result = addmod(0x4880cc52dcc54b2faba1842b31deb46b62253b8071494980ca953dc8f7e955a, mulmod(result, theta, modulus), modulus);
		result = addmod(0xc4338791c388ea87719dd202afd38799d1dc750b4cf9ccd62c4d243f0ad7dd9, mulmod(result, theta, modulus), modulus);
		result = addmod(0x1326a5ad52a6afe595365d8203dea5b318d825d0672167696edf79554821e62a, mulmod(result, theta, modulus), modulus);
		result = addmod(0x346c171d64290b4610d54545c649b9567733a2eb9a4bdab96814424ba24841ae, mulmod(result, theta, modulus), modulus);
		result = addmod(0x371bfcbfd70e41fbbd53f3a4ef579ef687e557541b8ac3d2136957e4eee4cc72, mulmod(result, theta, modulus), modulus);
		result = addmod(0xb8774292d77c46a3d57e46821f3f941e18d3ebc356bcc7fa067fb4a4e378521, mulmod(result, theta, modulus), modulus);
		result = addmod(0x2ff07f11a4bbe6e2c7ee03552aee1b778a970687cccc9803ec1be1c62249e587, mulmod(result, theta, modulus), modulus);
