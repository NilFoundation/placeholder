...
library modular_permutation_argument_circuit5_chunk10_lpc{
    uint256 constant modulus = 28948022309329048855892746252171976963363056481941560715954676764349967630337;
    uint256 constant permutation_size = 31;
    uint256 constant special_selectors_offset = 31 * 0x40;
    uint256 constant table_values_offset = 31 * 0x40 + 0x80;
    bytes constant zero_indices = hex"00400b6000a00100016001c00220028002e0034003a00400046004c00520058005e0064006a00700076007c00820088008e0094009a00a000a600ac00b20";
    uint16 constant max_quotient_chunks = 10;
    uint16 constant permutaion_parts = 4;
    ...
    function verify(
        ...
        state.V_P_value = basic_marshalling.get_uint256_be(blob, table_values_offset + 2944);
        state.h = 1;
        state.g = 1;
        ...
        for(uint256 i = 0; i < permutation_size;){
            ...
            if( cur == max_quotient_chunks - 1 ){
                state.current_value = basic_marshalling.get_uint256_be(blob, table_values_offset + 2944 + 0x40 + (chunk<<5));
                F[1] = addmod(
                    F[1],
                    mulmod(
        ...
        state.previous_value = state.current_value;
        state.current_value = basic_marshalling.get_uint256_be(blob, table_values_offset + 2944 + 0x20);
        F[1] = addmod(
            F[1],
            addmod(
                mulmod(state.previous_value,state.g, modulus),
