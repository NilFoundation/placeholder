...
#define BOOST_TEST_MODULE typed_message_view
...
BOOST_AUTO_TEST_CASE(message_views_detach_their_content) {
    ...
    auto msg2 = msg1;
    ACTOR_REQUIRE(msg1.cptr() == msg2.cptr());
    ACTOR_REQUIRE(msg1.match_elements<int, int, int, std::string>());
    typed_message_view<int, int, int, std::string> view {msg1};
    ACTOR_REQUIRE(msg1.cptr() != msg2.cptr());
}

BOOST_AUTO_TEST_CASE(message_views_allow_access_via_get) {
    auto msg = make_message(1, 2, 3, "four");
    ACTOR_REQUIRE(msg.match_elements<int, int, int, std::string>());
...
BOOST_AUTO_TEST_CASE(message_views_allow_mutating_elements) {
    auto msg1 = make_message(1, 2, 3, "four");
    auto msg2 = msg1;
    ACTOR_REQUIRE(msg1.match_elements<int, int, int, std::string>());
    typed_message_view<int, int, int, std::string> view {msg1};
    get<0>(view) = 10;
    BOOST_CHECK_EQUAL(msg1.get_as<int>(0), 10);
    BOOST_CHECK_EQUAL(msg2.get_as<int>(0), 1);
}
