...
#define BOOST_TEST_MODULE binary_serializer
...
#define CHECK_SAVE(type, value, ...) BOOST_CHECK_EQUAL(save(type {value}), byte_buffer({__VA_ARGS__}))
...
BOOST_AUTO_TEST_CASE(concatenation) {
    SUBTEST("calling f(a, b) writes a and b into the buffer in order") {
        BOOST_CHECK_EQUAL(save(int8_t {7}, int16_t {-32683}), byte_buffer({7_b, 0x80_b, 0x55_b}));
        BOOST_CHECK_EQUAL(save(int16_t {-32683}, int8_t {7}), byte_buffer({0x80_b, 0x55_b, 7_b}));
    }
    SUBTEST("calling f(a) and then f(b) is equal to calling f(a, b)") {
        ...
        BOOST_CHECK_EQUAL(data, byte_buffer({7_b, 0x80_b, 0x55_b}));
    }
    ...
    SUBTEST("arrays behave like tuples") {
        int8_t xs[] = {1, 2, 3};
        BOOST_CHECK_EQUAL(save(xs), byte_buffer({1_b, 2_b, 3_b}));
    }
}
...
BOOST_AUTO_TEST_CASE(binary_serializer_picks_up_inspect_functions) {
    ...
    SUBTEST("custom struct") {
        ...
        test_data value {-345, -1234567890123456789ll, 3.45, 54.3, ts, test_enum::b, "Lorem ipsum dolor sit amet."};
