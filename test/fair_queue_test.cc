...
struct request {
    ...
    request(unsigned weight, unsigned index, Func &&h) :
        fqent(fair_queue_ticket(weight, 0)), handle(std::move(h)), index(index) {
    }
...
public:
    test_env(unsigned capacity) :
        _fg(fair_group::config(capacity, std::numeric_limits<int>::max())), _fq(_fg, fair_queue::config()) {
    }
    ...
    unsigned tick(unsigned n = 1) {
        ...
        _fq.dispatch_requests(
            [](fair_queue_entry &ent) { boost::intrusive::get_parent_from_member(&ent, &request::fqent)->submit(); });
