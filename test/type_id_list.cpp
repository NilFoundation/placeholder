...
BOOST_AUTO_TEST_CASE(lists_are_comparable) {
    ...
    ACTOR_CHECK_LESS(xs, ys);
    BOOST_CHECK_EQUAL(make_type_id_list<add_atom>(), make_type_id_list<add_atom>());
    ACTOR_CHECK_NOT_EQUAL(make_type_id_list<add_atom>(), make_type_id_list<ok_atom>());
}

ACTOR_TEST(make_type_id_list constructs a list from types) {
...
ACTOR_TEST(type ID lists are convertible to strings) {
