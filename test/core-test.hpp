...
using float_actor = nil::actor::typed_actor<nil::actor::reacts_to<float>>;
...
[[maybe_unused]] inline bool operator==(const dummy_struct &x, const dummy_struct &y) {
    return x.a == y.a && x.b == y.b;
}

template<class Inspector>
decltype(auto) inspect(Inspector &f, dummy_struct &x) {
    return f(nil::actor::meta::type_name("dummy_struct"), x.a, x.b);
}
...
[[maybe_unused]] inline bool operator==(const my_request &x, const my_request &y) {
...
[[maybe_unused]] inline bool operator==(const raw_struct &lhs, const raw_struct &rhs) {
...
decltype(auto) inspect(Inspector &f, test_data &x) {
    return f(nil::actor::meta::type_name("test_data"), x.i32, x.i64, x.f32, x.f64, x.ts, x.te, x.str);
}

[[maybe_unused]] inline bool operator==(const test_data &x, const test_data &y) {
...
#define ADD_TYPE_ID(type) ACTOR_ADD_TYPE_ID(core_test, type)
#define ADD_ATOM(atom_name) ACTOR_ADD_ATOM(core_test, atom_name)

ACTOR_BEGIN_TYPE_ID_BLOCK(core_test, nil::actor::first_custom_type_id)

ADD_TYPE_ID((nil::actor::stream<int32_t>))
ADD_TYPE_ID((nil::actor::stream<std::string>))
ADD_TYPE_ID((nil::actor::stream<std::pair<level, std::string>>))
...
ACTOR_END_TYPE_ID_BLOCK(core_test)
