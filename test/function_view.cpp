...
#define BOOST_TEST_MODULE function_view
...
BOOST_AUTO_TEST_CASE(empty_function_fiew) {
    function_view<calculator> f;
    BOOST_CHECK_EQUAL(f(10, 20), sec::bad_function_call);
}
...
BOOST_AUTO_TEST_CASE(tuple_res_function_view) {
    auto f = make_function_view(system.spawn(simple_doubler));
    BOOST_CHECK_EQUAL(f(10), std::make_tuple(10, 10));
}

BOOST_AUTO_TEST_CASE(cell_function_view) {
    auto f = make_function_view(system.spawn(simple_cell));
    BOOST_CHECK_EQUAL(f(get_atom_v), 0);
    f(put_atom_v, 1024);
    BOOST_CHECK_EQUAL(f(get_atom_v), 1024);
}
