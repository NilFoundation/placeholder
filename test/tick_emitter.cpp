...
BOOST_AUTO_TEST_CASE(ticks) {
    ...
    ACTOR_MESSAGE("trigger 3 more ticks");
...
BOOST_AUTO_TEST_CASE(timeouts) {
    ...
    ACTOR_MESSAGE("advance until the first 5-tick-period ends");
    now += interval * 5;
    auto bitmask = tctrl.timeouts(now, {5, 7});
    BOOST_CHECK_EQUAL(bitmask, 0x01u);
    ACTOR_MESSAGE("advance until the first 7-tick-period ends");
    now += interval * 2;
    bitmask = tctrl.timeouts(now, {5, 7});
    BOOST_CHECK_EQUAL(bitmask, 0x02u);
    ACTOR_MESSAGE("advance until both tick period ends");
    now += interval * 7;
    bitmask = tctrl.timeouts(now, {5, 7});
    BOOST_CHECK_EQUAL(bitmask, 0x03u);
    ACTOR_MESSAGE("advance until both tick period end multiple times");
    now += interval * 21;
    bitmask = tctrl.timeouts(now, {5, 7});
    BOOST_CHECK_EQUAL(bitmask, 0x03u);
    ACTOR_MESSAGE("advance without any timeout");
    now += interval * 1;
    bitmask = tctrl.timeouts(now, {5, 7});
...
BOOST_AUTO_TEST_CASE(next_timeout) {
    ...
    ACTOR_MESSAGE("advance until the second 5-tick-period ends");
    now = start + timespan(7 * interval);
    next = tctrl.next_timeout(now, {5, 7});
    BOOST_CHECK_EQUAL(next, start + timespan((2 * 5) * interval));
    ACTOR_MESSAGE("advance until the second 7-tick-period ends");
